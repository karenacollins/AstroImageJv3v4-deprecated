...
//----------------------------------------------------
    ...
    static JMenuItem savedatamenuitem, saveimagemenuitem, savedataconfigmenuitem, saveplotconfigmenuitem,
            opendatamenuitem, openplotconfigmenuitem, opendataconfigmenuitem, saveplottemplatemenuitem, openplottemplatemenuitem,
            opentablemenuitem, transposetablemenuitem, addnewcolsfromplotmenuitem, addnewcolsfromastroCCmenuitem;
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            if (((e.getModifiers() & MouseEvent.BUTTON2_MASK) != 0) && button2Drag && !e.isShiftDown() && !e.isControlDown() && !e.isAltDown()) {                                                          // measure distance and report in Results Table
                ...
                IJ.log("x=" + fourPlaces.format(xval) + ", y=" + fourPlaces.format(yval) +
                        ", dx=" + fourPlaces.format(dxval) + ", dy=" + fourPlaces.format(dyval));
            }
...
//                                if (Data_Processor.running) Data_Processor.setTable(table);
                ...
                {                                                                                     //undo delete selected table row
                    ...
                    if (selectedRowEnd <= lastRow - binSize[firstCurve]) {  //restore all rows other than the last one OR next to last one when when binze > 1
                        ...
                        for (int i = 0; i < lines.length; i++) {
                            tpanel.appendWithoutUpdate(lines[i]);
                        }
    ...
    static void findTables(boolean forceUpdate) {
        ...
        if (tables == null || tables.length == 0) {
            ...
            for (int i = 0; i < tables.length; i++)
                if (tables[i].equals("B&C") || tables[i].startsWith("Log") || tables[i].equals("Errors"))
                    j++;
            ...
            if (requestedTableName != null) {
                for (int i = 0; i < filteredTables.length; i++) {
                    if (requestedTableName.equals(filteredTables[i])) {
...
//        Prefs.set("plot2.tableName",tableName);
    ...
    static public void updateColumnLists() {
        int j = 0;
        for (int i = 0; i < unfilteredColumns.length; i++)
            if (unfilteredColumns[i].equals("Label") || unfilteredColumns[i].equals("image"))
                j++;
        ...
        columnswd[1] = "default";
        for (int i = 2; i < columnswd.length; i++)
            columnswd[i] = columns[i - 1];
    }
    ...
    static public void sortColumns() {
        ...
        int j = 1;
        String pcol;
        for (int k = 0; k < pcols.length; k++) {
            pcol = pcols[k].toLowerCase().trim();
...
//                        clearPlot();
        ...
        if (n > maxColumnLength) {
            maxColumnLength = n > 2 * maxColumnLength ? n : 2 * maxColumnLength;
            setupDataBuffers();
        }
        unfilteredColumns = table.getColumnHeadings().split("\t");
        if (!Arrays.equals(unfilteredColumns, oldUnfilteredColumns)) {
...
//                        coeffs[curve] = null;
        ...
        for (int curve = 0; curve < maxCurves; curve++) {
            if (xlabel[curve].trim().length() == 0 ||
                    (xlabel[curve].equalsIgnoreCase("default") && xlabeldefault.trim().length() == 0)) {
                for (int j = 0; j < nn[curve]; j++)
                    x[curve][j] = (double) (j + 1);
...
//                                            showOpErrors[curve] = false;
                    ...
                    for (int j = 0; j < nn[curve]; j++) {
                        ...
                        for (int k = excludedHeadSamples; k < (bucketSize + excludedHeadSamples); k++) {
                            ...
                            if (Double.isNaN(yin)) {
                                ...
                                if (numNaN == bucketSize) {
                                    ...
                                    if (hasErrors[curve])
                                        yerr[curve][j] = Double.NaN;
                                    if (operatorIndex[curve] != 0)
                                        yop[curve][j] = Double.NaN;
                                    if (detrendFitIndex[curve] != 0) {
                                        for (int v = 0; v < maxDetrendVars; v++) {
                        ...
                        } else if (operatorIndex[curve] == 1)  //divide by
                        {
                            if (yop[curve][j] == 0) {
                                ...
                                if (hasErrors[curve] || hasOpErrors[curve]) {
                                    yerr[curve][j] = Math.sqrt(((yerr[curve][j] * yerr[curve][j]) / (yop[curve][j] * yop[curve][j])) +
                                            ((y[curve][j] * y[curve][j] * yoperr[curve][j] * yoperr[curve][j]) /
                                                    (yop[curve][j] * yop[curve][j] * yop[curve][j] * yop[curve][j])));  //yMultiplierFactor*
                                }
                        ...
                        {
                            if (hasErrors[curve] || hasOpErrors[curve])
                                yerr[curve][j] = Math.sqrt(yop[curve][j] * yop[curve][j] * yerr[curve][j] * yerr[curve][j] +
                                        y[curve][j] * y[curve][j] * yoperr[curve][j] * yoperr[curve][j]); // /yMultiplierFactor;
                            y[curve][j] = y[curve][j] * yop[curve][j];  // /yMultiplierFactor;
...
//                        IJ.log("");
                ...
                if (detrendFitIndex[curve] != 0) {
                    for (int v = 0; v < maxDetrendVars; v++) {
                        if (detrendIndex[curve][v] != 0) atLeastOne = true;
                    }
                }
                if (atLeastOne || detrendFitIndex[curve] == 9) {
                    ...
                    for (int v = 0; v < maxDetrendVars; v++) {
                        ...
                        if (v > 0) {
                            for (int u = 0; u < v; u++) {
                                if (detrendIndex[curve][u] == detrendIndex[curve][v])
                                    detrendPower[v]++;
                            }
                        }
                        if (detrendPower[v] > 1) {
                                    ...
                                    if (detrendFitIndex[curve] == 9) {
                                        ...
                                        for (int p = 0; p < maxFittedVars; p++) {
                                            if (isFitted[curve][p]) {
                                                ...
                                                if (p == 1) bestFit[curve][p] = Math.sqrt(priorCenter[curve][p]);
                                                else if (p == 4)
                                        ...
                                        if (useTransitFit[curve]) {
                                            ...
                                            bp[curve] = bestFit[curve][2] * Math.cos(bestFit[curve][4]) * (1.0 - (forceCircularOrbit[curve] ? 0.0 : eccentricity[curve] * eccentricity[curve])) /
                                                    (1.0 + (forceCircularOrbit[curve] ? 0.0 : eccentricity[curve]) * Math.sin(forceCircularOrbit[curve] ? 0.0 : omega[curve]));
                                            double bp2 = bp[curve] * bp[curve];
                                            t14[curve] = (orbitalPeriod[curve] / Math.PI * Math.asin(Math.sqrt((1.0 + bestFit[curve][1]) * (1.0 + bestFit[curve][1]) - bp2) /
                                                    (Math.sin(bestFit[curve][4]) * bestFit[curve][2])) * Math.sqrt(1.0 - (forceCircularOrbit[curve] ? 0.0 : eccentricity[curve] * eccentricity[curve])) /
                                                    (1.0 + (forceCircularOrbit[curve] ? 0.0 : eccentricity[curve]) * Math.sin(forceCircularOrbit[curve] ? 0.0 : omega[curve])));
                                            t23[curve] = (orbitalPeriod[curve] / Math.PI * Math.asin(Math.sqrt((1.0 - bestFit[curve][1]) * (1.0 - bestFit[curve][1]) - bp2) /
                                                    (Math.sin(bestFit[curve][4]) * bestFit[curve][2])) * Math.sqrt(1.0 - (forceCircularOrbit[curve] ? 0.0 : eccentricity[curve] * eccentricity[curve])) /
                                                    (1.0 + (forceCircularOrbit[curve] ? 0.0 : eccentricity[curve]) * Math.sin(forceCircularOrbit[curve] ? 0.0 : omega[curve])));
...
//                                }
                    ...
                    sigmaLabel[curve].setToolTipText("<html>RMS of model residuals (normalized).<br>" +
                            "Green Border: fit converged<br>" +
                            "Red Border: fit did not converge<br>" +
                ...
                if (mmag[curve]) {
                    ...
                    sigmaLabel[curve].setToolTipText("<html>RMS of model residuals (mmag).<br>" +
                            "Green Border: fit converged<br>" +
                            "Red Border: fit did not converge<br>" +
                            "Gray Border: no fit in this session</html>");
                ...
                if (normIndex[curve] == 0 && !mmag[curve]) {
                    sigmaLabel[curve].setText(sixPlaces.format(sigma[curve]));
                    sigmaLabel[curve].setToolTipText("<html>RMS of model residuals (raw).<br>" +
                            "WARNING: This is the standard deviation/RMS of the raw data from the model.<br>" +
                            "WARNING: Consider using either normalized or mmag output data.<br>" +
                            "Green Border: fit converged<br>" +
                            "Red Border: fit did not converge<br>" +
                            "Gray Border: no fit in this session</html>");
...
//                                    xOffset = (double)jd;
                        ...
                        if (useXColumnName) {
                            ...
                            else if (xlabel2[curve].trim().contains("JD"))
                                xlab = "Julian Date - " + xJD;
                            else
                                xlab = "Label Error";
                            xGoodLabel = true;
        ...
        if (xGoodLabel && (xExponent != 0) && showXScaleInfo)
            xlab += " x 1E" + xExponent;
...
//                                    yOffset = (double)jd;
                        ...
                        if (useYColumnName && operatorIndex[curve] != 5) {
                            ...
                            else if (ylabel[curve].trim().contains("JD"))
                                ylab = "Julian Date - " + yJD;
                            else
                                ylab = "Label Error";
                            yGoodLabel = true;
        ...
        if (yGoodLabel) {
            ...
            if (mmag[firstCurve] && showYmmagInfo)
                if (totalScaleFactor[firstCurve] == 1000.0)
                    ylab += " (mmag)";
                else
                    ylab += " (mag)";
...
//                            IJ.log("yModel2["+curve+"].length="+yModel2[curve].length);
                ...
                if (legendLeft)
                    ...
                    plot.setJustification(ij.process.ImageProcessor.RIGHT_JUSTIFY);
                else
                ...
                if (detrendFitIndex[curve] == 9 && useTransitFit[curve] && showResidual[curve] && showLResidual[curve] && residualShift[curve] > 0.0) {
                    llab = ylabel[curve] + " Residuals";
                    llab += " (RMS=" + (sigma[curve] >= 1.0 ? uptoThreePlaces.format(sigma[curve]) : uptoFivePlaces.format(sigma[curve])) + ") (chi^2/dof=" + uptoTwoPlaces.format(chi2dof[curve]) + ")";
                ...
                if (useColumnName[curve]) {
                    if (operatorIndex[curve] == 5) {
                        llab = xyc1label[curve] + " - " + xyc2label[curve] + " centroid distance";
                        boolean atLeastOne = false;
                        if (showLdetrendInfo && detrendFitIndex[curve] != 0) {
                            for (int v = 0; v < maxDetrendVars; v++) {
                                if (detrendIndex[curve][v] != 0) {
                                    if (atLeastOne)
                                        llab += "+" + detrendlabel[curve][v];
                                    else
                                        llab += " (" + detrendlabel[curve][v];
                                    atLeastOne = true;
                                }
                        ...
                        if (atLeastOne) {
                            if (detrendFitIndex[curve] == 9 && useTransitFit[curve]) {
                                llab += " detrended with transit fit)";
                            } else {
                                llab += " detrended)";
                            }
                        } else {
                        ...
                        if (showLnormInfo && normIndex[curve] != 0 && !mmag[curve] && !force[curve]) {
                            ...
                            llab += usePixelScale ? " (arcsecs)" : " (pixels)";
                        }
                    } else {
                        llab = ylabel[curve];
                        if (operatorIndex[curve] != 0)
                            llab += opSymbol[operatorIndex[curve]] + oplabel[curve];
                        boolean atLeastOne = false;
                        if (showLdetrendInfo && detrendFitIndex[curve] != 0) {
                            for (int v = 0; v < maxDetrendVars; v++) {
                                if (detrendIndex[curve][v] != 0) {
                    ...
                    if (mmag[curve] && showLmmagInfo)
                        if (mmag[firstCurve] && totalScaleFactor[firstCurve] == 1000)
                            llab += " (mmag)";
                        else if (totalScaleFactor[curve] == 1000)
                            llab += " (mmag)";
                        else
                            llab += " (mag)";
                    if (!force[curve])//&&(showLScaleInfo || showLShiftInfo))
                        llab += scaleShiftText(force[curve], showLScaleInfo, showLShiftInfo, mmag[curve], showLmmagInfo, totalScaleFactor[curve], totalShiftFactor[curve]);
                    else if (force[curve])//&&(showLRelScaleInfo || showLRelShiftInfo))
                        llab += scaleShiftText(force[curve], showLRelScaleInfo, showLRelShiftInfo, mmag[curve], showLmmagInfo, totalScaleFactor[curve], totalShiftFactor[curve]);
                    if ((binSize[curve] != 1) && showLBinInfo)
                        llab += " (bin size = " + binSize[curve] + ")";
                    if (showLSymbolInfo)
                ...
                if (detrendFitIndex[curve] == 9 && useTransitFit[curve] && showModel[curve] && showLTranParams[curve]) {
                    ...
                    llab += fourPlaces.format(bestFit[curve][1] * bestFit[curve][1]) + (lockToCenter[curve][1] ? "]" : "");
                    llab += ", " + (lockToCenter[curve][2] ? "[" : "") + "a/R*=" + onePlaces.format(bestFit[curve][2]) + (lockToCenter[curve][2] ? "]" : "");
                    llab += ", " + (lockToCenter[curve][4] ? "[" : "") + "i=" + onePlaces.format(bestFit[curve][4] * 180 / Math.PI) + (lockToCenter[curve][4] ? "]" : "");
                    llab += ", " + (lockToCenter[curve][3] ? "[" : "") + "Tc=" + uptoSixPlaces.format(bestFit[curve][3]) + (lockToCenter[curve][3] ? "]" : "");
                    llab += ", " + (lockToCenter[curve][5] ? "[" : "") + "u1=" + uptoTwoPlaces.format(bestFit[curve][5]) + (lockToCenter[curve][5] ? "]" : "");
                    llab += ", " + (lockToCenter[curve][6] ? "[" : "") + "u2=" + uptoTwoPlaces.format(bestFit[curve][6]) + (lockToCenter[curve][6] ? "]" : "") + ")";
                    drawLegendSymbol(ij.gui.Plot.LINE, modelLineWidth[curve] + ((showErrors[curve] || operatorIndex[curve] == 6) && (hasErrors[curve] || hasOpErrors[curve]) ? 1 : 0), modelColor[curve], legPosY, llab);
                    plot.addLabel(legendPosX, legPosY, llab);
                    legPosY += 18. / plotSizeY;
                }
...
//                    plotImage.setProcessor("Plot of "+tableName,ip);
            ...
            if (mml.length > 0)
                for (int i = 0; i < mml.length; i++)
                    plotImageCanvas.removeMouseMotionListener(mml[i]);

            MouseWheelListener[] mwl = plotImageCanvas.getMouseWheelListeners();
            if (mwl.length > 0)
                for (int i = 0; i < mwl.length; i++)
                    plotImageCanvas.removeMouseWheelListener(mwl[i]);

            MouseWheelListener[] mwl3 = plotWindow.getMouseWheelListeners();
            if (mwl3.length > 0)
                for (int i = 0; i < mwl3.length; i++)
                    plotWindow.removeMouseWheelListener(mwl3[i]);

            MouseListener[] ml = plotImageCanvas.getMouseListeners();
            if (ml.length > 0)
                for (int i = 0; i < ml.length; i++)
                    plotImageCanvas.removeMouseListener(ml[i]);
...
//                    }
    ...
    static void addNewAstroData() {
        ...
        useGJDButton.addActionListener(new ActionListener() {
        ...
        useHJDButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useGJD = false;
                useHJD = true;
                useBJD = false;
        ...
        useManualRaDecButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useTableRaDec = false;
        ...
        useTableRaDecButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useTableRaDec = true;
    ...
    public static void updateStack() {
        ...
        if (iplus != null) {
            ...
            if (iw != null && imstack != null && imstack.getSize() > 1 &&
                    ((row1Slice > 0 && row1Slice <= imstack.getSize()) || imstack.getSize() == table.getCounter())) {
    ...
    static void updateSlice() {
        Thread t = new Thread() {
            public void run() {
                ...
                else if (selectedRow >= imstack.getSize()) selectedRow = imstack.getSize() - 1;
                int newSlice = 0;
                if (sliceCol != MeasurementTable.COLUMN_NOT_FOUND)
...
//                iplus.updateAndDraw();
    ...
    static public void zoomControl(int screenX, int screenY, int magChangeSteps) {
        if (screenX > Plot.LEFT_MARGIN && screenX <= plotSizeX - Plot.RIGHT_MARGIN &&
                screenY >= Plot.TOP_MARGIN && screenY <= plotSizeY - Plot.BOTTOM_MARGIN) {
            if (zoom == 0.0) {
                mouseX = screenX - Plot.LEFT_MARGIN;
                mouseY = screenY - Plot.TOP_MARGIN;
//                        totalPanOffsetX=0.0;
//                        totalPanOffsetY=0.0;
//                        newPanOffsetX=0.0;
//                        newPanOffsetY=0.0;
...
//        useTwoLineLegend = false;
        ...
        addAstroDataFrameWasShowing = false;
        usePixelScale = true;
        deletedRowList = new ArrayList<String>();
        useBoldedDatum = true;
...
//        thouSep = symbols.getGroupingSeparator();
        ...
        integerspinnerscalelist = new String[]
                {
                        "                    1",
                        "                    2",
                        "                    3",
...
//                        showErr[i] = false;
    ...
    static String scaleShiftText(boolean relative, boolean showScale, boolean showShift, boolean usemmag, boolean showmmaginfo, double scale, double shift) {
        ...
        if (showScale && showmmaginfo && mmag[firstCurve] && usemmag && totalScaleFactor[firstCurve] == 1000 && scale != 1000) {
            ...
            if (((scale > 0.001) && (scale < 1000.0)) || ((scale < -0.001) && (scale > -1000.0)))
                scaleText += " x(" + dformatter.format(scale) + ")";
            else if (usemmag && showmmaginfo && (scale == 1000))
                scaleText += "";
            else
                scaleText += " x(" + Eformatter.format(scale) + ")";
        } else if (relative) {
            if (!showScale && !showShift) scaleText = " (arbitrarily scaled and shifted)";
        ...
        if (showShift) {
            ...
            if ((shift >= 0.01) && (shift < 100.0))
                shiftText += dformatter.format(shift);
            else if (shift != 0.0)
    ...
    static String convertToText(double increment) {
        ...
        else if (increment == 0.005) return "                    0.005";
    ...
    static String intConvertToText(int increment) {
        ...
        else if (increment == 2) return "                    2";
        else if (increment == 3) return "                    3";
        else if (increment == 4) return "                    4";
        else if (increment == 5) return "                    5";
        else if (increment == 6) return "                    6";
        else if (increment == 7) return "                    7";
        else if (increment == 8) return "                    8";
        else if (increment == 9) return "                    9";
        else if (increment == 10) return "                  10";
        else if (increment == 25) return "                  25";
        else if (increment == 50) return "                  50";
        else if (increment == 100) return "                100";
        else if (increment == 250) return "                250";
        else if (increment == 500) return "                500";
        else if (increment == 1000) return "              1000";
...
//                    IJ.log("value="+value);
            ...
            if (spinner.equals(vmarker2spinner)) {
                double vm1Value = ((Double) vmarker1spinner.getValue()).doubleValue();
...
//                mainpanelgroupe.setBorder(BorderFactory.createTitledBorder("Scaling Options"));
        ...
        opentablemenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                selectAnotherTableCanceled = false;
        ...
        opendatamenuitem.setToolTipText("<html>" + "default input format = tab delimited, or use" + "<br>" +
                "filename.csv = comma delimited" + "<br>" +
                "filename.prn or filename.spc = space delimted" + "<br>" +
                "---------------------------------------------" + "<br>" +
                "first line should be column headings delimited as stated above" + "<br>" +
                "lines starting with # are considered comments and ignored, except" + "<br>" +
                "the last comment before the first data line can be headings" + "</html>");
        ...
        appenddatarowsmenuitem.setToolTipText("<html>" + "default input format = tab delimited, or use" + "<br>" +
                ...
                "lines starting with # are considered comments and ignored, except" + "<br>" +
                "the last comment before the first data line can be headings" + "</html>");
        appenddatarowsmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
        ...
        appenddatacolumnsmenuitem.setToolTipText("<html>" + "default input format = tab delimited, or use" + "<br>" +
                ...
                "lines starting with # are considered comments and ignored, except" + "<br>" +
                "the last comment before the first data line can be headings" + "</html>");
        appenddatacolumnsmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
        ...
        openplotconfigmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                openConfig(false);
            }
        });
        ...
        opendataconfigmenuitem.setToolTipText("<html>" + "opens a previously saved measurement table" + "<br>" +
                "from a user selected file and attempts to" + "<br>" +
                "open a plot configuration file with the" + "<br>" +
                "same name but ending in .plotcfg" + "</html>");
        opendataconfigmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
        ...
        savedatasubsetmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
        ...
        saveimagejpgmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                savePlotImageAsJpg();
            }
        });
        ...
        saveplotconfigmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                saveConfig(false);
            }
        });
...
//                filemenu.add(createNEBLCPlotMenuItem);
        ...
        saveAllWithOptionsMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                saveAllDialog();
            }
        });
        ...
        opendatasetCB.addItemListener(new ItemListener() {
        ...
        rememberwindowlocationsCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
        ...
        showtooltipsCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED) {
                    showToolTips = false;
                } else if (e.getStateChange() == ItemEvent.SELECTED) {
        ...
        useBoldedDatumCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
...
//                preferencesmenu.add(setverticalmarkertextmenuitem);
        ...
        changePriorityColumnsmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                changePriorityColumns();
            }
        });
        ...
        changepixelscalemenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                changePixelScale();
                updatePlot(updateNoFits());
            }
        ...
        changemaxdatalengthmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                changeMaxDataLength();
                updatePlot(updateAllFits());
            }
        ...
        usedefaultsettingsCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    useDefaultSettings = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
        ...
        addnewcolsfromplotmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                int c = selectCurve();
                if (c < 0) return;
                addNewColumn(c, true);
        ...
        addnewcolsfromastroCCmenuitem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (addAstroDataFrame != null) {
                    addAstroDataFrameWasShowing = false;
                    closeAddAstroDataFrame();
...
//                xaxismenu.addSeparator();
        ...
        invertyaxisCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                ...
                for (int c = 0; c < maxCurves; c++) {
                    customshiftspinner[c].setModel(new SpinnerNumberModel(new Double(force[c] ? autoShiftFactor[c] * 100 : customShiftFactor[c]), null, null,
        ...
        negateMagCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    negateMag = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
...
//                mainpanelgroupa.add (ycolumnslabel);
        ...
        xdatacolumndefault.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        maxcurvesmodel = new SpinnerNumberModel(new Integer(maxCurves), new Integer(1), null, new Integer(1));
        ...
        maxcurvesspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                savePreferences();
                maxCurves = ((Integer) maxcurvesspinner.getValue()).intValue();
        ...
        maxcurvesspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) maxcurvesspinner.getValue()).intValue() - e.getWheelRotation());
                if (newValue > 0) maxcurvesspinner.setValue(newValue);
                else maxcurvesspinner.setValue(1);
            }
        });
...
//                maxdetrendvarspanel.setBorder(BorderFactory.createLineBorder(Color.lightGray,1));
        ...
        maxdetrendvarsspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                savePreferences();
                closeFitFrames();
                maxDetrendVars = ((Integer) maxdetrendvarsspinner.getValue()).intValue();
        ...
        maxdetrendvarsspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) maxdetrendvarsspinner.getValue()).intValue() - e.getWheelRotation());
                if (newValue > 0) maxdetrendvarsspinner.setValue(newValue);
                else maxdetrendvarsspinner.setValue(1);
            }
        });
        ...
        mmagsubpanel.setToolTipText("<html>Set the number of starting samples averaged to calculate relative magnitude reference level.<br>" +
                "Set to zero to calculate magnitude from the raw data (i.e. now division by a reference value).<br>" +
                "A reference level region can also be defined using 'Norm/Mag Ref' on the Y-data panel.</html>");
        mmagrefsmodel = new SpinnerNumberModel(new Integer(mmagrefs), new Integer(0), null, new Integer(1));
        ...
        mmagrefsspinner.setToolTipText("<html>Set the number of starting samples averaged to calculate relative magnitude reference level.<br>" +
                "Set to zero to calculate magnitude from the raw data (i.e. now division by a reference value).<br>" +
                "A reference level region can also be defined using 'Norm/Mag Ref' on the Y-data panel.</html>");
        mmagrefsspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                mmagrefs = ((Integer) mmagrefsspinner.getValue()).intValue();
                updatePlot(updateNoFits());
            }
        });
        mmagrefsspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) mmagrefsspinner.getValue()).intValue() - e.getWheelRotation());
        ...
        xstepspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (xWidth < 0.0001) xWidth = 0.0001;
                double value = IJU.getTextSpinnerDoubleValue(xstepspinner);//Double.parseDouble((String)xstepspinner.getValue());
                if (Double.isNaN(value)) return;
                xStep = value;
                vmarker1spinner.setModel(new SpinnerNumberModel(new Double(vMarker1Value), null, null, new Double(xStep)));
                vmarker2spinner.setModel(new SpinnerNumberModel(new Double(vMarker2Value), null, null, new Double(xStep)));
                xwidthspinner.setModel(new SpinnerNumberModel(new Double(xWidth), new Double(0.0001), null, new Double(xStep)));
                xmaxspinner.setModel(new SpinnerNumberModel(new Double(xMax), null, null, new Double(xStep)));
                xminspinner.setModel(new SpinnerNumberModel(new Double(xMin), null, null, new Double(xStep)));
                mfmarker1spinner.setModel(new SpinnerNumberModel(new Double(mfMarker1Value), null, null, new Double(xStep)));
                dmarker1spinner.setModel(new SpinnerNumberModel(new Double(dMarker1Value), null, null, new Double(xStep)));
                dmarker2spinner.setModel(new SpinnerNumberModel(new Double(dMarker2Value), null, null, new Double(xStep)));
                dmarker3spinner.setModel(new SpinnerNumberModel(new Double(dMarker3Value), null, null, new Double(xStep)));
                dmarker4spinner.setModel(new SpinnerNumberModel(new Double(dMarker4Value), null, null, new Double(xStep)));
        ...
        showVMarker1CB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    showVMarker1 = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
        ...
        vmarker1spinnermodel = new SpinnerNumberModel(new Double(vMarker1Value), null, null, new Double(xStep));
...
//                vmarker1spinner.setEditor(editor);
        ...
        vmarker1spinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                vmarker1spinner.setValue(((Double) vmarker1spinner.getValue()).doubleValue()
        ...
        showVMarker2CB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    showVMarker2 = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
        ...
        vmarker2spinnermodel = new SpinnerNumberModel(new Double(vMarker2Value), null, null, new Double(xStep));
        ...
        vmarker2spinner.setToolTipText("<html>" + "Enter vertical Marker 2 x-axis location" + "<br>" +
                "or enter UT time in HH:MM or HH:MM:SS format and press 'Enter'" + "<br>" +
                "---------------------------------------------" + "<br>" +
                "Right click to set spinner stepsize" + "</html>");
        vmarker2spinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
        ...
        vmarker2spinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                vmarker2spinner.setValue(((Double) vmarker2spinner.getValue()).doubleValue()
                        - e.getWheelRotation() * xStep);
            }
        });
...
//                        titlegroup.setBorder (BorderFactory.createEmptyBorder(10,10,10,10));
        ...
        if (!useTitle) noTitleButton.setSelected(true);
        else noTitleButton.setSelected(false);
        titleradiopanelgroup.add(noTitleButton);
        JRadioButton useTitleButton = new JRadioButton("Custom");
        useTitleButton.setFont(p11);
        if (useTitle) useTitleButton.setSelected(true);
        else useTitleButton.setSelected(false);
        ...
        noTitleButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useTitle = false;
                updatePlot(updateNoFits());
            }
        });
        useTitleButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useTitle = true;
                updatePlot(updateNoFits());
            }
        ...
        titlePosYSlider.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) titlePosYSlider.getValue()).intValue() + e.getWheelRotation());
        ...
        Hashtable<Integer, JLabel> titlePosYLabel = new Hashtable<Integer, JLabel>();
        ...
        titlePosXSlider.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) titlePosXSlider.getValue()).intValue() + e.getWheelRotation());
        ...
        Hashtable<Integer, JLabel> titlePosXLabel = new Hashtable<Integer, JLabel>();
        ...
        if (!useSubtitle) noSubtitleButton.setSelected(true);
        else noSubtitleButton.setSelected(false);
        ...
        if (useSubtitle) useSubtitleButton.setSelected(true);
        else useSubtitleButton.setSelected(false);
        ...
        noSubtitleButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useSubtitle = false;
                updatePlot(updateNoFits());
            }
        });
        useSubtitleButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useSubtitle = true;
                updatePlot(updateNoFits());
            }
        ...
        subtitlePosYSlider.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                JSlider slider = (JSlider) ev.getSource();
        ...
        subtitlePosYSlider.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) subtitlePosYSlider.getValue()).intValue() + e.getWheelRotation());
        ...
        Hashtable<Integer, JLabel> subtitlePosYLabel = new Hashtable<Integer, JLabel>();
        ...
        subtitlePosXSlider.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                JSlider slider = (JSlider) ev.getSource();
        ...
        subtitlePosXSlider.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) subtitlePosXSlider.getValue()).intValue() + e.getWheelRotation());
        ...
        Hashtable<Integer, JLabel> subtitlePosXLabel = new Hashtable<Integer, JLabel>();
        ...
        legendshowscalingCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
        ...
        legendshowrelscalingCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    showLRelScaleInfo = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
        ...
        legendshowrelshiftCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    showLRelShiftInfo = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
        ...
        legendshowsymbolCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    showLSymbolInfo = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
                    showLSymbolInfo = true;
                updatePlot(updateNoFits());
            }
        });
...
//                                SpringUtil.makeCompactGrid (legendradiopanelgroup, 1, legendradiopanelgroup.getComponentCount(), 1,1,1,1);
        ...
        legendPosYSlider.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                JSlider slider = (JSlider) ev.getSource();
        ...
        legendPosYSlider.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) legendPosYSlider.getValue()).intValue() + e.getWheelRotation());
        ...
        Hashtable<Integer, JLabel> legendPosYLabel = new Hashtable<Integer, JLabel>();
        ...
        legendPosXSlider.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                JSlider slider = (JSlider) ev.getSource();
        ...
        legendPosXSlider.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) legendPosXSlider.getValue()).intValue() + e.getWheelRotation());
        ...
        showxscalingCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
        ...
        if (!useXCustomName && !useXColumnName)
            xnoneButton.setSelected(true);
        else
            xnoneButton.setSelected(false);
        ...
        if (useXColumnName)
            xcolumnNameButton.setSelected(true);
        else
            xcolumnNameButton.setSelected(false);
        ...
        if (!useXColumnName && useXCustomName)
            xcustomNameButton.setSelected(true);
        else
            xcustomNameButton.setSelected(false);
        ...
        xnoneButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        xcustomNameButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useXColumnName = false;
                useXCustomName = true;
                updatePlot(updateNoFits());
        ...
        xcolumnNameButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useXColumnName = true;
                useXCustomName = false;
                updatePlot(updateNoFits());
        ...
        xlegendconfigureButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                xaxispopup.show(xlegendconfigureButton, mainpanel.getX(), mainpanel.getY() + 25);
            }
        });
...
//                                SpringUtil.makeCompactGrid (xlegendradiopanelgroup, 1, xlegendradiopanelgroup.getComponentCount(), 1,1,1,1);
        ...
        showyscalingCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
        ...
        showyshiftCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
        ...
        showynormCB.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    showYNormInfo = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
        ...
        if (!useYCustomName && !useYColumnName)
            ynoneButton.setSelected(true);
        else
            ynoneButton.setSelected(false);
        ...
        if (useYColumnName)
            ycolumnNameButton.setSelected(true);
        else
            ycolumnNameButton.setSelected(false);
        ...
        if (!useYColumnName && useYCustomName)
            ycustomNameButton.setSelected(true);
        else
            ycustomNameButton.setSelected(false);
        ...
        ylegendconfigureButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                yaxispopup.show(ylegendconfigureButton, mainpanel.getX(), mainpanel.getY() + 25);
            }
        });
        ...
        ynoneButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        ycustomNameButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useYColumnName = false;
                useYCustomName = true;
                updatePlot(updateNoFits());
        ...
        ycolumnNameButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                useYColumnName = true;
                useYCustomName = false;
                updatePlot(updateNoFits());
...
//                                SpringUtil.makeCompactGrid (ylegendradiopanelgroup, 1, 3, 0,0,0,0);
        ...
        trimdataheadstepspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                excludedHeadSamplesStep = Integer.parseInt(((String) trimdataheadstepspinner.getValue()).trim());
                trimdataheadspinner.setModel(new SpinnerNumberModel(new Integer(excludedHeadSamples), new Integer(0), null, new Integer(excludedHeadSamplesStep)));
        ...
        trimdataheadmodel = new SpinnerNumberModel(new Integer(excludedHeadSamples), new Integer(0), null, new Integer(excludedHeadSamplesStep));
        ...
        trimdataheadspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                excludedHeadSamples = ((Integer) trimdataheadspinner.getValue()).intValue();
                Prefs.set("plot.excludedHeadSamples", excludedHeadSamples);
                updatePlot(updateAllFits());
            }
        });
        trimdataheadspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) trimdataheadspinner.getValue()).intValue() - e.getWheelRotation());
        ...
        trimdatatailstepspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                excludedTailSamplesStep = Integer.parseInt(((String) trimdatatailstepspinner.getValue()).trim());
                trimdatatailspinner.setModel(new SpinnerNumberModel(new Integer(excludedTailSamples), new Integer(0), null, new Integer(excludedTailSamplesStep)));
        ...
        trimdatatailmodel = new SpinnerNumberModel(new Integer(excludedTailSamples), new Integer(0), null, new Integer(excludedTailSamplesStep));
        ...
        trimdatatailspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                excludedTailSamples = ((Integer) trimdatatailspinner.getValue()).intValue();
                Prefs.set("plot.excludedTailSamples", excludedTailSamples);
                updatePlot(updateAllFits());
            }
        });
        trimdatatailspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) trimdatatailspinner.getValue()).intValue() - e.getWheelRotation());
        ...
        autoxButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                autoScaleX = true;
                useFirstX = false;
                updatePlot(updateAllFits());
        ...
        firstxButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                autoScaleX = false;
                useFirstX = true;
                updatePlot(updateAllFits());
        ...
        customxButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                autoScaleX = false;
                useFirstX = false;
                updatePlot(updateAllFits());
        ...
        grabautoxbutton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                xMin = xautoscalemin;
        ...
        if (xWidth < 0.0001) xWidth = 0.0001;
        xwidthspinnermodel = new SpinnerNumberModel(new Double(xWidth), new Double(0.0001), null, new Double(xStep));
        ...
        xwidthspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                xWidth = ((Double) xwidthspinner.getValue()).doubleValue();
                updatePlot(updateNoFits());
            }
        });
        xwidthspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newvalue = ((Double) xwidthspinner.getValue()).doubleValue() - e.getWheelRotation() * xStep;
        ...
        xmaxspinnermodel = new SpinnerNumberModel(new Double(xMax), null, null, new Double(xStep));
        ...
        xmaxspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                xMax = ((Double) xmaxspinner.getValue()).doubleValue();
                updatePlot(updateNoFits());
            }
        });
        xmaxspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                xmaxspinner.setValue(((Double) xmaxspinner.getValue()).doubleValue()
                        - e.getWheelRotation() * xStep);
            }
        });
        ...
        xmultipliermodel = new SpinnerNumberModel(new Integer(xExponent), null, null, new Integer(1));
        ...
        xmultiplierspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                xExponent = ((Integer) xmultiplierspinner.getValue()).intValue();
                updatePlot(updateNoFits());
            }
        });
        xmultiplierspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                xmultiplierspinner.setValue(new Integer(((Integer) xmultiplierspinner.getValue()).intValue()
                        - e.getWheelRotation()));
            }
        });
        ...
        grabfirstxbutton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                xMin = firstXmin;
        ...
        xminspinnermodel = new SpinnerNumberModel(new Double(xMin), null, null, new Double(xStep));
        ...
        xminspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                xMin = ((Double) xminspinner.getValue()).doubleValue();
                updatePlot(updateNoFits());
            }
        });
        xminspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                xminspinner.setValue(((Double) xminspinner.getValue()).doubleValue()
                        - e.getWheelRotation() * xStep);
            }
        });
        ...
        autoyButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                autoScaleY = true;
                updatePlot(updateNoFits());
            }
        });
        customyButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                autoScaleY = false;
                updatePlot(updateNoFits());
            }
        ...
        grabautoybutton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                yMin = yautoscalemin;
        ...
        ymaxstepspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(ymaxstepspinner);
                if (Double.isNaN(value)) return;
                yMaxStep = value;
                ymaxspinner.setModel(new SpinnerNumberModel(new Double(yMax), null, null, new Double(yMaxStep)));
        ...
        ymaxspinnermodel = new SpinnerNumberModel(new Double(yMax), null, null, new Double(yMaxStep));
        ...
        ymaxspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                yMax = ((Double) ymaxspinner.getValue()).doubleValue();
                updatePlot(updateNoFits());
            }
        });
        ymaxspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                ymaxspinner.setValue(((Double) ymaxspinner.getValue()).doubleValue()
                        - e.getWheelRotation() * yMaxStep);
            }
        });
        ...
        ymultipliermodel = new SpinnerNumberModel(new Integer(yExponent), null, null, new Integer(1));
        ...
        ymultiplierspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                yExponent = ((Integer) ymultiplierspinner.getValue()).intValue();
                updatePlot(updateNoFits());
            }
        });
        ymultiplierspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                ymultiplierspinner.setValue(new Integer(((Integer) ymultiplierspinner.getValue()).intValue()
                        - e.getWheelRotation()));
            }
        });
        ...
        yminstepspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(yminstepspinner);
                if (Double.isNaN(value)) return;
                yMinStep = value;
                yminspinner.setModel(new SpinnerNumberModel(new Double(yMin), null, null, new Double(yMinStep)));
        ...
        yminspinnermodel = new SpinnerNumberModel(new Double(yMin), null, null, new Double(yMinStep));
        ...
        yminspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                yMin = ((Double) yminspinner.getValue()).doubleValue();
                updatePlot(updateNoFits());
            }
        });
        yminspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                yminspinner.setValue(((Double) yminspinner.getValue()).doubleValue()
                        - e.getWheelRotation() * yMinStep);
            }
        });
        ...
        plotheightstepspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                plotSizeYStep = Integer.parseInt(((String) plotheightstepspinner.getValue()).trim());
                plotheightspinner.setModel(new SpinnerNumberModel(new Integer(plotSizeY), new Integer(1), null, new Integer(plotSizeYStep)));
        ...
        plotheightspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                plotSizeY = ((Integer) plotheightspinner.getValue()).intValue();
                updatePlot(updateNoFits());
            }
        });
        plotheightspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) plotheightspinner.getValue()).intValue() - e.getWheelRotation() * plotSizeYStep);
        ...
        plotwidthstepspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                plotSizeXStep = Integer.parseInt(((String) plotwidthstepspinner.getValue()).trim());
                plotwidthspinner.setModel(new SpinnerNumberModel(new Integer(plotSizeX), new Integer(1), null, new Integer(plotSizeXStep)));
        ...
        plotwidthspinnermodel = new SpinnerNumberModel(new Integer(plotSizeX), new Integer(1), null, new Integer(plotSizeXStep));
        ...
        plotwidthspinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                plotSizeX = ((Integer) plotwidthspinner.getValue()).intValue();
                updatePlot(updateNoFits());
            }
        });

        plotwidthspinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) plotwidthspinner.getValue()).intValue() - e.getWheelRotation() * plotSizeXStep);
...
//                vmarker1spinner.setEditor(editor);
        ...
        mfmarker1spinner.setToolTipText("<html>" + "Enter meridian flip time in x-axis units" + "<br>" +
                "or enter UT time in HH:MM or HH:MM:SS format and press 'Enter'" + "<br>" +
                "---------------------------------------------" + "<br>" +
                "Right click to set spinner stepsize" + "</html>");
        mfmarker1spinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                showMFMarkersCB.setSelected(true);
                checkForUT(mfmarker1spinner);
                mfMarker1Value = ((Double) mfmarker1spinner.getValue()).doubleValue();
                updatePlot(updateAllFits());
            }
        });
        mfmarker1spinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                mfmarker1spinner.setValue(((Double) mfmarker1spinner.getValue()).doubleValue()
                        - e.getWheelRotation() * xStep);
            }
        });
        ...
        dmarker1spinnermodel = new SpinnerNumberModel(new Double(dMarker1Value), null, null, new Double(xStep));
...
//                vmarker1spinner.setEditor(editor);
        ...
        dmarker1spinner.setToolTipText("<html>" + "Enter detrend/normalize left trim location" + "<br>" +
                "or enter UT time in HH:MM or HH:MM:SS format and press 'Enter'" + "<br>" +
                "---------------------------------------------" + "<br>" +
                "SHORTCUT: &lt;Ctrl&gt;&lt;Shift&gt; Left Click or Drag in plot" + "<br>" +
                "---------------------------------------------" + "<br>" +
                "Right click to set spinner stepsize" + "</html>");
        dmarker1spinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                checkForUT(dmarker1spinner);
                dMarker1Value = ((Double) dmarker1spinner.getValue()).doubleValue();
                keepMarkersInOrder(1);
                updatePlot(updateAllFits());
            }
        });
        dmarker1spinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                if (useDMarker1) {
                    dmarker1spinner.setValue(((Double) dmarker1spinner.getValue()).doubleValue()
                            - e.getWheelRotation() * xStep);
                }
            }
        });
...
//                vmarker1spinner.setEditor(editor);
        ...
        dmarker2spinner.setToolTipText("<html>" + "Enter detrend/normalize left marker location" + "<br>" +
                "or enter UT time in HH:MM or HH:MM:SS format and press 'Enter'" + "<br>" +
                "---------------------------------------------" + "<br>" +
                "SHORTCUT: &lt;Ctrl&gt; Left Click or Drag in plot    " + "<br>" +
                "---------------------------------------------" + "<br>" +
                "Right click to set spinner stepsize" + "</html>");
        dmarker2spinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
        ...
        dmarker2spinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
...
//                vmarker1spinner.setEditor(editor);
        ...
        dmarker3spinner.setToolTipText("<html>" + "Enter detrend/normalize right marker location" + "<br>" +
                "or enter UT time in HH:MM or HH:MM:SS format and press 'Enter'" + "<br>" +
                "---------------------------------------------" + "<br>" +
        ...
        dmarker3spinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
...
//                dmarker4title.setTitlePosition(TitledBorder.BOTTOM);
        ...
        dmarker4spinner.setToolTipText("<html>" + "Enter detrend/normalize right trim location" + "<br>" +
                "or enter UT time in HH:MM or HH:MM:SS format and press 'Enter'" + "<br>" +
                "---------------------------------------------" + "<br>" +
                "SHORTCUT: &lt;Ctrl&gt;&lt;Shift&gt; Right Click or Drag in plot" + "<br>" +
                "---------------------------------------------" + "<br>" +
                "Right click to set spinner stepsize" + "</html>");
        dmarker4spinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                checkForUT(dmarker4spinner);
                dMarker4Value = ((Double) dmarker4spinner.getValue()).doubleValue();
                keepMarkersInOrder(4);
                updatePlot(updateAllFits());
            }
        });
        dmarker4spinner.addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                if (useDMarker4) {
                    dmarker4spinner.setValue(((Double) dmarker4spinner.getValue()).doubleValue()
                            - e.getWheelRotation() * xStep);
                }
            }
        });
...
//                        constructOtherGroupTopLabels(mainsubpanelgroupb);
        ...
        for (cur = 0; cur < maxCurves; cur++) {
            ...
            if (useWideDataPanel)
                constructOtherGroup(mainsubpanelgroupa, cur);
            else
...
//                mainsubpanelgroup.add (ycolumnlabelsub);
        ...
        magToFluxlabel.setToolTipText("<HTML>Enable to convert magnitude-based Y-input data (Y-data, Y-error, Y-operand, and Y-operand-error) to flux for additional calculations .<BR>" +
...
//                legendslabelgroup.add(Box.createGlue());
        ...
        ycolumnlabelconfigureButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                openDataHelpPanel();
            }
        });
...
//                legendslabelgroup.add(Box.createGlue());
        ...
        useerrorlabel.setToolTipText("<HTML>Show automatic Y-error bars when dataset has predefined error available.<BR>" +
                "Compatible datasets are: Source-Sky_XX, rel_flux_XX, tot_C_cnts.</HTML>");
...
//                mainsubpanelgroup.add (operrorlabel);
    ...
    static void constructOtherGroupBottomLabels(JPanel mainsubpanelgroup) {
        ...
        detrendlabelA.setToolTipText("<html>Fit and/or detrend Y-data using a user-defined or fitted constant value times the 'Trend Data' values.<br>" +
                "If transit fit mode is selected, an exoplanet transit model will be fit between the left and right fit markers.<br>" +
                "If trend dataset(s) are also selected in transit fit mode, detrending will be fit simultaneously with the transit model.<br>" +
                "--------------------------------------------------------------------------------------------------<br>" +
                "In transit fit mode, right-click the 'Fit Mode' box to redisplay the 'Transit Fit Settings' panel.<br>" +
        ...
        if (maxDetrendVars > 1) {
            ...
            detrendlabelB.setToolTipText("<html>Click buttons to show the corresponding trend dataset coefficient and name.<br>" +
                    "If a trend dataset has been selected and a fit mode is enabled,<br>" +
                    "the button background will display green. The background will be gray otherwise.<br>" +
                    "---------------------------------------------------------------------------<br>" +
                    "Select a trend dataset more than once to increase the power of its polynomial fit.<br>" +
                    "One occurance of a trend dataset -> linear fit, two occurances -> quadratic fit, etc.<br>" +
                    "---------------------------------------------------------------------------<br>" +
                    "If 'Show tooltips help' is enabled under the 'Preferences' menu,<br>" +
                    "a mouse-over of a radio button shows the column set for that variable.<br>" +
                    "If a radio button is gray and a tooltip shows a column name enclosed in square brackets,<br>" +
                    "that column will be recalled if the radio button is clicked again.<br>" +
                    "---------------------------------------------------------------------------<br>" +
                    "Set the Fit and Normalize region markers at the bottom of the Multi-plot Main panel.<br>" +
                    "---------------------------------------------------------------------------<br>" +
                    "SHORTCUTS:<br>" +
                    "Click an already selected button again to alternately enable/disable the corresponding trend variable<br>" +
                    "&lt;SHIFT&gt;Click - selects this trend column to be displayed for all rows<br>" +
                    "&lt;CTRL&gt;Click - copies the selected trend dataset to all rows<br>" +
        ...
        detrendlabelC.setToolTipText("<html>White background indicates user-selected value, gray background indicates fitted constant value.<br>" +
                "Set the Fit and Normalize region markers at the bottom of the Multi-plot Main panel.</html>");
        ...
        detrendlabelD.setToolTipText("<html>Trend data used to detrend Y-data.<br>" +
                "---------------------------------------------------------------------------<br>" +
                "Select a 'Trend Dataset' more than once to increase the power of its polynomial fit.<br>" +
                "One occurance of a Trend Dataset -> linear fit, two occurances -> quadratic fit, etc.<br>" +
                "---------------------------------------------------------------------------<br>" +
...
//                mainsubpanelgroup.add(customscalelabel4);
        ...
        normlabel.setToolTipText("<html>Select the region of data used to normalize the dataset.<br>" +
                "If 'Out Mag' is selected, the mean of the region is used as the reference<br>" +
                "level when converting the plotted data to relative magnitude.<br>" +
                "Set the Fit and Normalize region markers at the bottom of the Multi-plot Main panel.</html>");
        ...
        mmaglabel.setToolTipText("<html>Plot data in relative magnitude referenced to the average of the first n data samples,<br>" +
                "where n is set by the 'Rel. Mag. Reference' spinner on the 'Multi-plot Main' window.<br>" +
                "If a 'Norm/Mag Ref' region is selected, the mean of the region is used as the reference<br>" +
...
//                legendslabelgroup.add(Box.createGlue());
        ...
        legendslabelconfigureButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
...
//                savecolumnpanelgroup[c].setBorder(BorderFactory.createEmptyBorder());
        ...
        savecolumnbutton[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
...
//                SpringUtil.makeCompactGrid (savecolumnpanelgroup[c], 1, savecolumnpanelgroup[c].getComponentCount(), 0,0,0,0);
        ...
        autoscaleincludebox[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    ASInclude[c] = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
...
//                    xdatacolumn[c].setPrototypeDisplayValue("1234567890123456");
        xdatacolumn[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        fromMagBox[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    fromMag[c] = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
...
//                    ydatacolumn[c].setPrototypeDisplayValue("1234567890123456");
        ydatacolumn[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        operatorselection[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
...
//                    operatorcolumn[c].setPrototypeDisplayValue("1234567890123456");
        operatorcolumn[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
...
//                        markercolorselection[c].setPreferredSize(new Dimension(100, 25));
        ...
        markercolorselection[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
...
//                        markersymbolselection[c].setPreferredSize(new Dimension(100, 25));
        ...
        markersymbolselection[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        binsizespinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                binSize[c] = ((Integer) binsizespinner[c].getValue()).intValue();
                Prefs.set("plot.binSize" + c, binSize[c]);
                updatePlot(updateOneFit(c));
            }
        });
        binsizespinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer(((Integer) binsizespinner[c].getValue()).intValue() - e.getWheelRotation());
...
//                customscalepanelgroup[c].setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(new Color(122,138,153)),BorderFactory.createLineBorder(new Color(184,207,229))));
        ...
        detrendtypecombobox[c].setToolTipText("<html>Select 'user' to manually select detrend coefficient or select region type to auto-fit<br>" +
                "for the best detrend coefficient(s) for the selected trend data column(s).<br>" +
                "If transit fit mode is selected, an exoplanet transit model will be fit between the left and right fit markers.<br>" +
                "If trend dataset(s) are also selected in transit fit mode, detrending will be fit simultaneously with the transit model.<br>" +
                "-------------------------------------------------------------------------------------------------------<br>" +
                "In transit fit mode, right click the 'Fit Mode' box to redisplay the 'Transit Fit Settings' panel.<br>" +
        ...
        detrendtypecombobox[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
...
//                            }
        ...
        detrendfactormodel[c] = new SpinnerNumberModel(new Double(detrendFactor[c][detrendVarDisplayed[c]]), null, null, new Double(detrendFactorStep[c][detrendVarDisplayed[c]]));
        detrendfactorspinner[c] = new JSpinner(detrendfactormodel[c]);
        detrendfactorspinner[c].setFont(p11);
        detrendfactorspinner[c].setToolTipText("<HTML>Y-dataset values are detrended by applying the product(s)<BR>" +
                "of the coefficient(s) and the corresponding 'Trend Dataset(s)'.<BR>" +
                "Right click to set spinner stepsize.</HTML>");
        ...
        detrendfactorspinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (detrendFitIndex[c] == 1) {
                    detrendFactor[c][detrendVarDisplayed[c]] = ((Double) detrendfactorspinner[c].getValue()).doubleValue();
                    Prefs.set("plot.detrendFactor[" + c + "][" + detrendVarDisplayed[c] + "]", detrendFactor[c][detrendVarDisplayed[c]]);
                    updatePlot(updateOneFit(c));
                }
            }
        });
        detrendfactorspinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                if (detrendFitIndex[c] == 1) {
        ...
        detrendfactorstepspinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                ...
                detrendfactorspinner[c].setModel(new SpinnerNumberModel(new Double(detrendFactor[c][detrendVarDisplayed[c]]), null, null, new Double(detrendFactorStep[c][detrendVarDisplayed[c]])));
                detrendfactorspinner[c].setEditor(new JSpinner.NumberEditor(detrendfactorspinner[c], "########0.#########"));
            }
        });
        ...
        detrendbox[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                ...
                detrendVarButton[c][detrendVarDisplayed[c]].setToolTipText(detrendlabel[c][detrendVarDisplayed[c]].trim().equals("") ?
        ...
        normtypecombobox[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        usemmagbox[c].setToolTipText("<html>Plot data in relative magnitude referenced to the average of the first n data samples,<br>" +
                ...
                "If a 'Norm/Mag Ref' region is selected, the mean of the region is used as the reference<br>" +
                "level when converting the plotted data to relative magnitude.</html>");
        usemmagbox[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    mmag[c] = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
...
//                grabautopanel[c] = new JPanel (new SpringLayout());
        ...
        grabautobutton[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
...
//                        customshiftspinner[c].setValue(customShiftFactor[c]);
                ...
                customscalespinner[c].setModel(new SpinnerNumberModel(new Double(customScaleFactor[c]), null, null, new Double(customScaleStep[c])));
                ...
                customshiftspinner[c].setModel(new SpinnerNumberModel(new Double(customShiftFactor[c]), null, null, new Double(customShiftStep[c])));
                customshiftspinner[c].setEditor(new JSpinner.NumberEditor(customshiftspinner[c], "########0.#########"));
                ((JSpinner.DefaultEditor) customshiftspinner[c].getEditor()).getTextField().addMouseListener(shiftSpinnerMouseListener);
...
//                customscalepanelgroup[c].add (grabautopanel[c]);
        ...
        forcebox[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED)
                    force[c] = false;
                else if (e.getStateChange() == ItemEvent.SELECTED)
                ...
                customscalespinner[c].setModel(new SpinnerNumberModel(new Double(force[c] ? autoScaleFactor[c] * 100.0 : customScaleFactor[c]), null, null, new Double(force[c] ? autoScaleStep[c] : customScaleStep[c])));
                customscalespinner[c].setEditor(new JSpinner.NumberEditor(customscalespinner[c], "########0.#########"));
                customscalestepspinner[c].setValue(convertToText(force[c] ? autoScaleStep[c] : customScaleStep[c]));
                customshiftspinner[c].setModel(new SpinnerNumberModel(new Double(force[c] ? autoShiftFactor[c] * 100 : customShiftFactor[c]), null, null, new Double(force[c] ? autoShiftStep[c] : customShiftStep[c])));
                customshiftspinner[c].setEditor(new JSpinner.NumberEditor(customshiftspinner[c], "########0.#########"));
                ((JSpinner.DefaultEditor) customshiftspinner[c].getEditor()).getTextField().addMouseListener(shiftSpinnerMouseListener);
...
//                forcebox[c].setHorizontalAlignment(JLabel.CENTER);
        ...
        customscalemodel[c] = new SpinnerNumberModel(new Double(force[c] ? autoScaleFactor[c] * 100 : customScaleFactor[c]), null, null, new Double(force[c] ? autoScaleStep[c] : customScaleStep[c]));
        customscalespinner[c] = new JSpinner(customscalemodel[c]);
        customscalespinner[c].setFont(p11);
        customscalespinner[c].setEditor(new JSpinner.NumberEditor(customscalespinner[c], "########0.#########"));
        customscalespinner[c].setToolTipText("<HTML>If 'Page Rel' is deselected, the Y-dataset values are multiplied by this factor<BR>" +
                "(and by the factor of 10 specified in the 'Y-axis' box of the 'Multi-plot Main' panel).<BR>" +
                "If 'Page Rel' is selected, the dataset values are arbitrarily scaled to fit on a 'Scale' percentage of the Y-axis range.<BR>" +
                "Right click to set spinner stepsize.</HTML>");
...
//                customscaleformat[c].setMaximumFractionDigits(9);
        ...
        customscalespinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (force[c])
                    autoScaleFactor[c] = ((Double) customscalespinner[c].getValue()).doubleValue() / 100.0;
                else
                    customScaleFactor[c] = ((Double) customscalespinner[c].getValue()).doubleValue();
                if (Double.isNaN(autoScaleFactor[c]) || Double.isInfinite(autoScaleFactor[c])) autoScaleFactor[c] = 1.0;
                if (Double.isNaN(customScaleFactor[c]) || Double.isInfinite(customScaleFactor[c]))
                    customScaleFactor[c] = 1.0;
                updatePlot(updateNoFits());
        ...
        customscalespinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                if (force[c])
                    customscalespinner[c].setValue(((Double) customscalespinner[c].getValue()).doubleValue() - e.getWheelRotation() * autoScaleStep[c]);
                else
                    customscalespinner[c].setValue(((Double) customscalespinner[c].getValue()).doubleValue() - e.getWheelRotation() * customScaleStep[c]);
                if (Double.isNaN(((Double) customscalespinner[c].getValue()).doubleValue()) || Double.isInfinite(((Double) customscalespinner[c].getValue()).doubleValue())) {
        ...
        customscalestepspinner[c].setValue(convertToText(force[c] ? autoScaleStep[c] : customScaleStep[c]));
        customscalestepspinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (force[c]) {
...
//                        customscaleformat[c].setMaximumFractionDigits(9);
        ...
        customshiftmodel[c] = new SpinnerNumberModel(new Double(force[c] ? autoShiftFactor[c] * 100 : customShiftFactor[c]), null, null,
                new Double(force[c] ? invertYAxisSign * autoShiftStep[c] : invertYAxisSign * customShiftStep[c]));
...
//                customshiftformat[c].setMaximumFractionDigits(6);
        ...
        customshiftspinner[c].setToolTipText("<HTML>If 'Page Rel' is deselected, the 'Shift' value is added to the scaled Y-dataset values before plotting.<BR>" +
                "If 'Page Rel' is selected, the dataset values are arbitrarily shifted by 'Shift' percentage of the Y-axis plot range.<BR>" +
                "Use 'CONTROL' and 'SHIFT' click modifiers to select and shift multiple curves simultaneously.<BR>" +
                "Right click to set spinner stepsize.</HTML>");
        customshiftspinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (customshiftspinner[c].isEnabled()) {
                    ...
                    for (int curve = 0; curve < maxCurves; curve++) {
                        if (curve != c && customshiftspinner[curve].getBorder().equals(greenBorder) && force[curve] == force[c]) {
                            ...
                            if (force[c]) {
                                autoShiftFactor[curve] = ((((Double) customshiftspinner[curve].getValue()).doubleValue() + (autoShiftFactor[c] > oldAutoShiftFactor ? 1.0 : -1.0) * autoShiftStep[c]) / 100.0);
                                if (Double.isNaN(autoShiftFactor[curve]) || Double.isInfinite(autoShiftFactor[curve]))
                                    autoShiftFactor[curve] = 0.0;
                                customshiftspinner[curve].setValue((((Double) customshiftspinner[curve].getValue()).doubleValue() + (autoShiftFactor[c] > oldAutoShiftFactor ? 1.0 : -1.0) * autoShiftStep[c]));
                                if (Double.isNaN(((Double) customshiftspinner[curve].getValue()).doubleValue()) || Double.isInfinite(((Double) customshiftspinner[curve].getValue()).doubleValue()))
                                    customshiftspinner[curve].setValue(0.0);
                            } else {
                                customShiftFactor[curve] = ((Double) customshiftspinner[curve].getValue()).doubleValue() + (customShiftFactor[c] > oldCustomShiftFactor ? 1.0 : -1.0) * customShiftStep[c];
                                if (Double.isNaN(customShiftFactor[curve]) || Double.isInfinite(customShiftFactor[curve]))
                                    customShiftFactor[curve] = 0.0;
                                customshiftspinner[curve].setValue(((Double) customshiftspinner[curve].getValue()).doubleValue() + (customShiftFactor[c] > oldCustomShiftFactor ? 1.0 : -1.0) * customShiftStep[c]);
                                if (Double.isNaN(((Double) customshiftspinner[curve].getValue()).doubleValue()) || Double.isInfinite(((Double) customshiftspinner[curve].getValue()).doubleValue()))
                                    customshiftspinner[curve].setValue(0.0);
                            }
                        }
        ...
        customshiftspinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                if (force[c])
                    customshiftspinner[c].setValue(((Double) customshiftspinner[c].getValue()).doubleValue() -
                            invertYAxisSign * e.getWheelRotation() * autoShiftStep[c]);
                else
                    customshiftspinner[c].setValue(((Double) customshiftspinner[c].getValue()).doubleValue() -
                            invertYAxisSign * e.getWheelRotation() * customShiftStep[c]);
                if (Double.isNaN(((Double) customshiftspinner[c].getValue()).doubleValue()) || Double.isInfinite(((Double) customshiftspinner[c].getValue()).doubleValue()))
            ...
            public void stateChanged(ChangeEvent ev) {
                ...
                for (int curve = start; curve < end; curve++) {
                    ...
                    customshiftspinner[curve].setModel(new SpinnerNumberModel(new Double(force[curve] ? autoShiftFactor[curve] * 100 : customShiftFactor[curve]), null, null,
                            new Double(force[curve] ? invertYAxisSign * autoShiftStep[curve] : invertYAxisSign * customShiftStep[curve])));
        ...
        shiftAboveBox[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED) {
...
//                legendslabelgroup.add(Box.createGlue());
        ...
        legendnoneButton[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        legendcustomNameButton[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        legendcolumnNameButton[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        if (useWideDataPanel)
            morelegendField[c].setPreferredSize(new Dimension(150, 20));
        else
...
//                SpringUtil.makeCompactGrid (morelegendradiopanelgroup[c], 1, morelegendradiopanelgroup[c].getComponentCount(), 0,0,0,0);
    ...
    static void constructDetrendVarRadioGroup(final int c, final int v) {
        ...
        detrendVarButton[c][v].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
...
//                                useFitDetrendCB[c][v].setSelected(true);
    ...
    static void createFitPanelCommonItems() {
        ...
        orbitalPeriodStepSpinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(orbitalPeriodStepSpinner);
                if (Double.isNaN(value)) return;
                orbitalPeriodStep = value;
                for (int c = 0; c < maxCurves; c++) {
                    orbitalPeriodSpinner[c].setModel(new SpinnerNumberModel(new Double(orbitalPeriod[c]), 0.001, null, new Double(orbitalPeriodStep)));
...
//ECCENTRICITY STEPSIZE POPUP
        ...
        eccentricityStepSpinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
        ...
        omegaStepSpinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
        ...
        teffStepSpinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
        ...
        jminuskStepSpinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(jminuskStepSpinner);
                if (Double.isNaN(value)) return;
                jminuskStep = value;
                for (int c = 0; c < maxCurves; c++) {
                    jminuskSpinner[c].setModel(new SpinnerNumberModel(new Double(((Double) jminuskSpinner[c].getValue()).doubleValue()), new Double(IJU.JminusK[0]), new Double(IJU.JminusK[IJU.JminusK.length - 1]), new Double(jminuskStep)));
        ...
        mStarStepSpinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(mStarStepSpinner);
                if (Double.isNaN(value)) return;
                mStarStep = value;
                for (int c = 0; c < maxCurves; c++) {
                    mStarSpinner[c].setModel(new SpinnerNumberModel(new Double(((Double) mStarSpinner[c].getValue()).doubleValue()), new Double(IJU.mStar[IJU.mStar.length - 1]), new Double(IJU.mStar[0]), new Double(mStarStep)));
        ...
        rStarStepSpinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(rStarStepSpinner);
                if (Double.isNaN(value)) return;
                rStarStep = value;
                for (int c = 0; c < maxCurves; c++) {
                    rStarSpinner[c].setModel(new SpinnerNumberModel(new Double(((Double) rStarSpinner[c].getValue()).doubleValue()), new Double(IJU.rStar[IJU.rStar.length - 1]), new Double(IJU.rStar[0]), new Double(rStarStep)));
        ...
        rhoStarStepSpinner.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(rhoStarStepSpinner);
                if (Double.isNaN(value)) return;
                rhoStarStep = value;
                for (int c = 0; c < maxCurves; c++) {
                    rhoStarSpinner[c].setModel(new SpinnerNumberModel(new Double(((Double) rhoStarSpinner[c].getValue()).doubleValue()), new Double(IJU.rhoStar[0]), new Double(IJU.rhoStar[IJU.rhoStar.length - 1]), new Double(rhoStarStep)));
    ...
    static void buildFitAndDetrendPopups(final int p) {
        ...
        priorCenterStepSpinner[p].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                ...
                for (int c = 0; c < maxCurves; c++) {
                    if (p == 0)
                        priorCenterSpinner[c][p].setModel(new SpinnerNumberModel(new Double(priorCenter[c][p]), new Double(0.0), null, new Double(priorCenterStep[p])));
                    else if (p == 1)
                        priorCenterSpinner[c][p].setModel(new SpinnerNumberModel(new Double(priorCenter[c][p]), new Double(0.0), null, new Double(priorCenterStep[p])));
                    else if (p == 2)
                        priorCenterSpinner[c][p].setModel(new SpinnerNumberModel(new Double(priorCenter[c][p]), new Double(0.0), null, new Double(priorCenterStep[p])));
                        ...
                        priorCenterSpinner[c][p].setModel(new SpinnerNumberModel(new Double(priorCenter[c][p]), null, null, new Double(priorCenterStep[p])));
        ...
        priorWidthStepSpinner[p].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(priorWidthStepSpinner[p]);
                if (Double.isNaN(value)) return;
                priorWidthStep[p] = value;
                for (int c = 0; c < maxCurves; c++) {
                    priorWidthSpinner[c][p].setModel(new SpinnerNumberModel(new Double(priorWidth[c][p]), new Double(0.0), null, new Double(priorWidthStep[p])));
        ...
        fitStepStepSpinner[p].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                double value = IJU.getTextSpinnerDoubleValue(fitStepStepSpinner[p]);
                if (Double.isNaN(value)) return;
                fitStepStep[p] = value;
                for (int c = 0; c < maxCurves; c++) {
                    fitStepSpinner[c][p].setModel(new SpinnerNumberModel(new Double(fitStep[c][p]), new Double(0.0), null, new Double(fitStepStep[p])));
...
//            fitPanel[c].setBorder(BorderFactory.createLineBorder(color[c], 2));
        fitPanel[c].setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(color[c], 2), table == null ? "No Table Selected" : ylabel[c].trim().equals("") ? "No Data Column Selected" : ylabel[c], TitledBorder.CENTER, TitledBorder.TOP, b12, Color.darkGray));
        FileDrop fileDrop = new FileDrop(fitPanel[c], BorderFactory.createEmptyBorder(), new FileDrop.Listener() {
            public void filesDropped(java.io.File[] files) {
                openDragAndDropFiles(files);
            }
        });
        ...
        baselinePriorCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
...
//            specifiedParmetersPanel.add(periodDummyLabel1);
        ...
        orbitalPeriodSpinner[c].setToolTipText("<html>Enter orbital period of eclipsing object.<br>" +
                "The orbital period of a transiting planet is not well constrained by<br>" +
                "a single transit light curve and must be specified from prior data.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        orbitalPeriodSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                orbitalPeriod[c] = ((Double) orbitalPeriodSpinner[c].getValue()).doubleValue();
        ...
        orbitalPeriodSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) orbitalPeriodSpinner[c].getValue()).doubleValue() - e.getWheelRotation() * orbitalPeriodStep;
                if (newValue < 0.001) orbitalPeriodSpinner[c].setValue(0.001);
                else orbitalPeriodSpinner[c].setValue(newValue);
            }
        });
...
//            specifiedParmetersPanel.add(eccentricityLabel);
        ...
        eccentricitySpinner[c].setToolTipText("<html>Enter eccentricity of eclipsing object's orbit.<br>" +
                "The eccentricity of a transiting planet's orbit is not well constrained by<br>" +
                "a transit light curve and must be specified from prior data.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        eccentricitySpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                eccentricity[c] = ((Double) eccentricitySpinner[c].getValue()).doubleValue();
                if (autoUpdateFit[c]) updatePlot(updateOneFit(c));
            }
        });
        eccentricitySpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) eccentricitySpinner[c].getValue()).doubleValue() - e.getWheelRotation() * eccentricityStep;
                if (eccentricitySpinner[c].isEnabled()) {
                    if (newValue < 0.0) eccentricitySpinner[c].setValue(0.0);
                    else if (newValue > 1.0) eccentricitySpinner[c].setValue(1.0);
                    else eccentricitySpinner[c].setValue(newValue);
                }
            }
        });
        ...
        omegaSpinner[c].setToolTipText("<html>Enter the argument of periastron of eclipsing object's orbit.<br>" +
                "The argument of periastron of a transiting planet's orbit is not well constrained by<br>" +
                "a transit light curve and must be specified from prior data.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        omegaSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                omega[c] = ((Double) omegaSpinner[c].getValue()).doubleValue();
                if (autoUpdateFit[c]) updatePlot(updateOneFit(c));
            }
        });
        omegaSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) omegaSpinner[c].getValue()).doubleValue() - e.getWheelRotation() * omegaStep;
                if (omegaSpinner[c].isEnabled()) {
                    if (newValue < -360.0) omegaSpinner[c].setValue(-360.0);
                    else if (newValue > 360.0) omegaSpinner[c].setValue(360.0);
                    else omegaSpinner[c].setValue(newValue);
                }
            }
        });
...
//            spectralTypeSelection[c].setForeground(color[c]);
        ...
        spectralTypeSelection[c].setToolTipText("<html>Select the host star spectral type to calculate the other<br>" +
                "stellar parameters and the planet radius.<br>" +
                "This setting is required only if a planetary radius estimation<br>" +
                "is to be calculated from the light curve fit.</html>");
        spectralTypeSelection[c].setMaximumRowCount(IJU.tStar.length);
        spectralTypeSelection[c].setPrototypeDisplayValue("123");
        spectralTypeSelection[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                if (!spectralTypeSelection[c].isEditable()) {
                    ...
                    else if (newValue > IJU.tStar[0]) teffSpinner[c].setValue(IJU.tStar[0]);
        ...
        teffSpinner[c].setToolTipText("<html>Enter the effective temperature of the host star in K and press enter<br>" +
                "to calculate the other stellar parameters and the planet radius.<br>" +
                "This value is required only if a planetary radius estimation<br>" +
                "is to be calculated from the light curve fit.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        teffSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
        ...
        teffSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) teffSpinner[c].getValue()).doubleValue() - e.getWheelRotation() * teffStep;
        ...
        jminuskSpinner[c].setToolTipText("<html>Enter J-K for the host star and press enter to calculate<br>" +
                "the other stellar parameters and the planet radius.<br>" +
                "This value is required only if a planetary radius estimation<br>" +
                "is to be calculated from the light curve fit.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        jminuskSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (jminuskSpinner[c].isEnabled()) {
                    teffSpinner[c].setValue(IJU.getTeffFromJminusK(((Double) jminuskSpinner[c].getValue()).doubleValue()));
                }
            }
        });
        jminuskSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) jminuskSpinner[c].getValue()).doubleValue() - e.getWheelRotation() * jminuskStep;
                if (jminuskSpinner[c].isEnabled()) {
                    if (newValue < IJU.JminusK[0]) jminuskSpinner[c].setValue(IJU.JminusK[0]);
                    else if (newValue > IJU.JminusK[IJU.JminusK.length - 1])
                        jminuskSpinner[c].setValue(IJU.JminusK[IJU.JminusK.length - 1]);
                    else jminuskSpinner[c].setValue(newValue);
                }
            }
        });
        ...
        rStarSpinner[c].setToolTipText("<html>Enter the radius of the host star and press enter to calculate<br>" +
                "the other stellar parameters and the planet radius.<br>" +
                "This value is required only if a planetary radius estimation<br>" +
                "is to be calculated from the light curve fit.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        rStarSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (rStarSpinner[c].isEnabled()) {
                    teffSpinner[c].setValue(IJU.getTeffFromRStar(((Double) rStarSpinner[c].getValue()).doubleValue()));
                }
            }
        });
        rStarSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) rStarSpinner[c].getValue()).doubleValue() - e.getWheelRotation() * rStarStep;
                if (rStarSpinner[c].isEnabled()) {
                    if (newValue < IJU.rStar[IJU.rStar.length - 1])
                        rStarSpinner[c].setValue(IJU.rStar[IJU.rStar.length - 1]);
                    else if (newValue > IJU.rStar[0]) rStarSpinner[c].setValue(IJU.rStar[0]);
                    else rStarSpinner[c].setValue(newValue);
                }
            }
        });
        ...
        mStarSpinner[c].setToolTipText("<html>Enter the mass of the host star and press enter to calculate<br>" +
                "the other stellar parameters and the planet radius.<br>" +
                "This value is required only if a planetary radius estimation<br>" +
                "is to be calculated from the light curve fit.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        mStarSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (mStarSpinner[c].isEnabled()) {
                    teffSpinner[c].setValue(IJU.getTeffFromMStar(((Double) mStarSpinner[c].getValue()).doubleValue()));
                }
            }
        });
        mStarSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) mStarSpinner[c].getValue()).doubleValue() - e.getWheelRotation() * mStarStep;
                if (mStarSpinner[c].isEnabled()) {
                    if (newValue < IJU.mStar[IJU.mStar.length - 1])
                        mStarSpinner[c].setValue(IJU.mStar[IJU.mStar.length - 1]);
                    else if (newValue > IJU.mStar[0]) mStarSpinner[c].setValue(IJU.mStar[0]);
                    else mStarSpinner[c].setValue(newValue);
                }
            }
        });
        ...
        rhoStarSpinner[c].setToolTipText("<html>Enter density of the host star in g/cm^3 and press enter to calculate<br>" +
                "the other stellar parameters and the planet radius.<br>" +
                "This value is required only if a planetary radius estimation<br>" +
                "is to be calculated from the light curve fit.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        rhoStarSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (rhoStarSpinner[c].isEnabled()) {
                    teffSpinner[c].setValue(IJU.getTeffFromRhoStar(((Double) rhoStarSpinner[c].getValue()).doubleValue()));
                }
            }
        });
        rhoStarSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) rhoStarSpinner[c].getValue()).doubleValue() - e.getWheelRotation() * rhoStarStep;
                if (rhoStarSpinner[c].isEnabled()) {
                    if (newValue < IJU.rhoStar[0]) rhoStarSpinner[c].setValue(IJU.rhoStar[0]);
                    else if (newValue > IJU.rhoStar[IJU.rhoStar.length - 1])
                        rhoStarSpinner[c].setValue(IJU.rhoStar[IJU.rhoStar.length - 1]);
                    else rhoStarSpinner[c].setValue(newValue);
                }
            }
        });
        ...
        useTransitFitCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED) {
...
//            fittedParametersPanel1.add(fittedParametersPanel1Label0);
        ...
        autoUpdatePriorsCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED) {
        ...
        extractPriorsButton[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (plotY[c]) {
        ...
        fittedColumnNameLabel4a.setToolTipText("<html>Enable to enter a custom minimization stepsize.<br>" +
        ...
        fittedColumnNameLabel4b.setToolTipText("<html>Enable to enter a custom minimization stepsize.<br>" +
                "Disable for automatic determination of minimizer stepsize.<br>" +
                "The minimizer is used to find the best light curve fit.</html>");
        ...
        t14Label[c].setToolTipText("<html>The duration of the transit (days).<br>" +
                ...
                "of 1<sup>st</sup> contact to time of 4<sup>th</sup> contact (t<sub>14</sub>).<br>" +
                "Green Border: fit converged<br>" +
                "Red Border: fit did not converge<br>" +
                "Gray Border: no fit in this session</html>");
        ...
        t23Label[c].setToolTipText("<html>The in-transit duration of the transit (days).<br>" +
                "This is the duration of the transit from time<br>" +
                "of 2<sup>nd</sup> contact to time of 3<sup>rd</sup> contact (t<sub>23</sub>).<br>" +
                "Green Border: fit converged<br>" +
                "Red Border: fit did not converge<br>" +
                "Gray Border: no fit in this session</html>");
        ...
        tauLabel[c].setToolTipText("<html>The transit ingress (and egress) duration (days).<br>" +
                "This is the duration of the transit ingress (and egress) from time<br>" +
                "of 1<sup>st</sup> contact to time of 2<sup>nd</sup> contact (t<sub>12</sub>) or in the <br>" +
                "case of egress t<sub>34</sub>. t<sub>12</sub> and t<sub>34</sub> are assumed to be equal.<br>" +
                "Green Border: fit converged<br>" +
                "Red Border: fit did not converge<br>" +
                "Gray Border: no fit in this session</html>");
        ...
        stellarDensityLabel[c].setToolTipText("<html>Host star density as derived from the light curve shape (g/cm^3).<br>" +
                "Density calculations are based on Seager & Mallen-Ornelas 2003 eqn 9.<br>" +
                "Green Border: fit converged<br>" +
                "Red Border: fit did not converge<br>" +
                "Gray Border: no fit in this session</html>");
        ...
        spectralTypeLabel[c].setToolTipText("<html>Host star spectral type estimated from light curved derived density.<br>" +
                "This convertion is approximate and assumes the star is on the main sequence.<br>" +
                "Green Border: fit converged<br>" +
                "Red Border: fit did not converge<br>" +
                "Gray Border: no fit in this session</html>");
        ...
        planetRadiusLabel[c].setToolTipText("<html>Planet radius in Jupiter Radii estimated from transit depth and host star radius as entered or calculated above. <br>" +
                "If available, enter the host star radius directly for the best estimate of planet radius.<br>" +
                "Otherwise, planet radius estimation assumes the host star is on the main sequence.<br>" +
                "Green Border: fit converged<br>" +
                "Red Border: fit did not converge<br>" +
                "Gray Border: no fit in this session</html>");
...
//            fittedColumnNameLabel3a.setPreferredSize(spinnerSize);
        ...
        detrendUseCustomStepLabel.setToolTipText("<html>Enable to enter a custom minimization stepsize.<br>" +
                "Disable for automatic determination of minimizer stepsize.<br>" +
                "The minimizer is used to find the best light curve fit.</html>");
//            fittedColumnNameLabel3a.setPreferredSize(spinnerSize);
        ...
        detrendStepSizeLabel.setToolTipText("<html>Enable to enter a custom minimization stepsize.<br>" +
                "Disable for automatic determination of minimizer stepsize.<br>" +
                "The minimizer is used to find the best light curve fit.</html>");
        ...
        bicLabel[c].setToolTipText("<html>Bayesian Information Criterion (BIC).<br>" +
                ...
                "The fit with the lowest BIC value indicates the preferred model.<br>" +
                "The difference in BIC values of two models indicates the significance<br>" +
                "of the preference of the model with the lowest BIC value:<br>" +
                "0-2 -> no preference<br>" +
                "2-6 -> positive preference<br>" +
                "6-10 -> strong preference<br>" +
                ">10 -> very strong preference</html>");
        ...
        showModelCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
        ...
        showLTranParamsCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
        ...
        modelColorSelection[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        modelLineWidthSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                modelLineWidth[c] = new Integer((Integer) modelLineWidthSpinner[c].getValue()).intValue();
                updatePlot(updateNoFits());
            }
        });
        modelLineWidthSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                if (modelLineWidthSpinner[c].isEnabled()) {
                    int newValue = new Integer((Integer) modelLineWidthSpinner[c].getValue()).intValue() - e.getWheelRotation();
        ...
        showResidualCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
        ...
        showLResidualCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
        ...
        showResidualErrorCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
        ...
        residualModelColorSelection[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        residualLineWidthSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                residualLineWidth[c] = new Integer((Integer) residualLineWidthSpinner[c].getValue()).intValue();
                updatePlot(updateNoFits());
            }
        });
        residualLineWidthSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                if (residualLineWidthSpinner[c].isEnabled()) {
                    int newValue = new Integer((Integer) residualLineWidthSpinner[c].getValue()).intValue() - e.getWheelRotation();
        ...
        residualSymbolSelection[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        residualColorSelection[c].addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
        ...
        residualShiftSpinner[c].setToolTipText("<html>Enter the amount to shift the residuals on the y-axis<br>" +
                "relative to the transit model baseline.<br>" +
                "---------------------------------------------<br>" +
                "Right click to set spinner stepsize</html>");
        residualShiftSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (force[c])
                    autoResidualShift[c] = ((Double) residualShiftSpinner[c].getValue()).doubleValue() / 100.0;
                else
                    residualShift[c] = ((Double) residualShiftSpinner[c].getValue()).doubleValue();
                updatePlot(updateNoFits());
            }
        });
        residualShiftSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
        ...
        autoUpdateFitCB[c].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED) {
        ...
        fitNowButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (plotY[c]) {
                    updatePlot(updateOneFit(c));
                } else {
...
//            toleranceSpinner[c].setComponentPopupMenu(priorCenterStepPopup[row]);
        toleranceSpinner[c].setToolTipText("<html>The minimization tolerance for the fit.<br>" +
                "The default value is 1.0E-10.</html>");
        toleranceSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
...
//                        toleranceSpinner[c].setValue(tolerance[c]);
        ...
        toleranceSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) toleranceSpinner[c].getValue()).doubleValue() - (e.getWheelRotation() / Math.abs(e.getWheelRotation())) * tolerance[c] * 0.0010;
                if (newValue >= Double.MIN_NORMAL && newValue <= Double.MAX_VALUE)
                    toleranceSpinner[c].setValue(newValue);
            }
        });
...
//            toleranceSpinner[c].setComponentPopupMenu(priorCenterStepPopup[row]);
        maxFitStepsSpinner[c].setToolTipText("<html>The maximum number of steps allowed to reach the minimization tolerance for the fit.<br>" +
                "The default value is 20,000 steps.</html>");
        maxFitStepsSpinner[c].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                maxFitSteps[c] = new Integer((Integer) maxFitStepsSpinner[c].getValue()).intValue();
                if (autoUpdateFit[c]) updatePlot(updateOneFit(c));
            }
        });
        maxFitStepsSpinner[c].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                int newValue = new Integer((Integer) maxFitStepsSpinner[c].getValue()).intValue() - e.getWheelRotation() * 1000;
...
//                IJ.log("yCenterAverage["+c+"]="+yDepthEstimate[c]);
    ...
    static void createFittedParametersRow(JPanel parentPanel, final int c, final int row, final String rowName, final String rowNameToolTipText, final double min, final double max) {
        ...
        if (isDetrend) {  //detrend parameter
            ...
            useFitDetrendCB[c][row - 7].addItemListener(new ItemListener() {
                public void itemStateChanged(ItemEvent e) {
            ...
            fitDetrendComboBox[c][row - 7].addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent ae) {
        ...
        bestFitLabel[c][row].setToolTipText("<html>Green Border: best fit parameter value from converged fit<br>" +
                ...
                "Gray Border: parameter has not been fit in this session");
...
//            rowNameLabel.setToolTipText(rowNameToolTipText);
        ...
        lockToCenterCB[c][row].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED) {
...
//            IJ.log("");
        ...
        if (row < 4 && !isDetrend) {
            priorCenterSpinner[c][row].setToolTipText("<html>The parameter's starting value for the fit.<br>" +
                    "---------------------------------------------<br>" +
                    "To enter a custom value, either disable 'Auto Update Priors' or enable 'Lock'.<br>" +
                    "---------------------------------------------<br>" +
                    "Right click to set spinner stepsize</html>");
        } else {
            priorCenterSpinner[c][row].setToolTipText("<html>The parameter's starting value for the fit.<br>" +
        ...
        priorCenterSpinner[c][row].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) priorCenterSpinner[c][row].getValue()).doubleValue() - e.getWheelRotation() * priorCenterStep[row];
...
//            rowNameLabel.setToolTipText(rowNameToolTipText);
        ...
        if (row < 2 && !isDetrend) {
            usePriorWidthCB[c][row].setToolTipText("<html>If enabled, the 'Prior Width' value constrains the parameter to be in<br>" +
                    ...
                    "---------------------------------------------------<br>" +
                    "To enter a custom 'Prior Width' value, either disable 'Auto Update Priors'<br>" +
                    "or enable 'Lock' to 'Prior Center'. </html>");
        } else {
            usePriorWidthCB[c][row].setToolTipText("<html>If enabled, the 'Prior Width' value constrains the parameter to be in<br>" +
        ...
        if (row < 2 && !isDetrend) {
            priorWidthSpinner[c][row].setToolTipText("<html>If 'Use' is enabled, the 'Prior Width' value constrains the parameter to be in<br>" +
                    "the range of the 'Prior Center' value plus/minus the 'Prior Width' value.<br>" +
                    "---------------------------------------------------<br>" +
                    "To set a custom value, either disable 'Auto Update Priors'<br>" +
                    "or enable 'Lock' to 'Prior Center'.<br>" +
                    "---------------------------------------------------<br>" +
                    "Right click to set spinner stepsize</html>");
        } else {
            priorWidthSpinner[c][row].setToolTipText("<html>If 'Use' is enabled, the 'Prior Width' value constrains the parameter to be in<br>" +
        ...
        priorWidthSpinner[c][row].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                priorWidth[c][row] = ((Double) priorWidthSpinner[c][row].getValue()).doubleValue();
                fitStepSpinner[c][row].setValue(getFitStep(c, row));
                if (autoUpdateFit[c]) updatePlot(updateOneFit(c));
            }
        });
        priorWidthSpinner[c][row].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) priorWidthSpinner[c][row].getValue()).doubleValue() - e.getWheelRotation() * priorWidthStep[row];
                if (priorWidthSpinner[c][row].isEnabled() && newValue >= 0.0)
                    priorWidthSpinner[c][row].setValue(newValue);
            }
        });
...
//            rowNameLabel.setToolTipText(rowNameToolTipText);
        ...
        useCustomFitStepCB[c][row].setToolTipText("<html>Enable to enter a custom minimization stepsize.<br>" +
                "Disable for automatic determination of minimizer stepsize.<br>" +
                "The minimizer is used to find the best light curve fit.<html>");
        useCustomFitStepCB[c][row].addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.DESELECTED) {
        ...
        fitStepSpinner[c][row].setToolTipText("<html>If enabled, the user can set a custom NelderMead minimization stepsize.<br>" +
                "If disabled, AIJ will automatically determination the minimizer stepsize.<br>" +
                "The minimizer is used to find the best light curve fit.</html>");
        fitStepSpinner[c][row].addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent ev) {
                if (useCustomFitStep[c][row])
                    fitStep[c][row] = ((Double) fitStepSpinner[c][row].getValue()).doubleValue();
                if (autoUpdateFit[c]) updatePlot(updateOneFit(c));
            }
        });
        fitStepSpinner[c][row].addMouseWheelListener(new MouseWheelListener() {
            public void mouseWheelMoved(MouseWheelEvent e) {
                double newValue = new Double((Double) fitStepSpinner[c][row].getValue()).doubleValue() - e.getWheelRotation() * fitStepStep[row];
...
//                chi2Label[c].paint(chi2Label[c].getGraphics());
    ...
    static void setFittedParametersBorderColorNewThread(final int c, final Border border) {
        Thread t = new Thread() {
            public void run() {
                setFittedParametersBorderColor(c, border);
            }
        };
        t.start();
        Thread.yield();
    ...
    static double getFitStep(int c, int row) {
        return useCustomFitStep[c][row] ? fitStep[c][row] > 0.0 ? fitStep[c][row] : defaultFitStep[row] :
                (usePriorWidth[c][row] && priorWidth[c][row] > 0.0 ?
        ...
        for (int ap = 0; ap < numAps; ap++) {
            ...
            if (showSaturationWarning || showLinearityWarning) {
                ...
                if (peakcol != ResultsTable.COLUMN_NOT_FOUND) {
                    ...
                    refStarCB[ap].setBackground(showSaturationWarning && isStarSaturated[ap] ? Color.RED :
                            (showLinearityWarning && isStarOverLinear[ap] ? darkYellow : darkGreen));
                } else {
...
//                    });
        ...
        if (table != null) {
            ...
            refstarlabel1.setToolTipText("<html>Updates T's and C's in the table column headings and the following table values:<br>" +
                    "rel_flux_Xx, rel_flux_err_Xx, rel_flux_SNR_Xx, tot_C_cnts, tot_C_err, </html>");
        }
        ...
        if (numAps > 0) {
            ...
            for (int r = 0; r < numAps; r++) {
                ...
                if (isRefStar[r]) {
                    absMagTF[r].setToolTipText("<html>Enter known magnitude and press ENTER to recalculate target star magnitudes.<br>" +
                            "Right click to open SIMBAD target(s) near coordinates" +
                            " (requires RA_C" + (r + 1) + " and Dec_C" + (r + 1) + " columns in table.");
                } else {
                    absMagTF[r].setToolTipText("<html>Right click to open SIMBAD target(s) near coordinates" +
                            " (requires RA_T" + (r + 1) + " and Dec_T" + (r + 1) + " columns in table.");
                }
...
//                    reminderPanel.add (reminderlabel);
                ...
                if ((e.getModifiers() & MouseEvent.BUTTON3_MASK) != 0) {
                    ...
                    if (col != ResultsTable.COLUMN_NOT_FOUND) {
                        ...
                        if (col != ResultsTable.COLUMN_NOT_FOUND) {
                            ...
                            IJ.beep();
                            IJ.showMessage("Error Accessing SIMBAD", "<html>Table column " + "DEC_" + (isRefStar[a] ? "C" : "T") + (a + 1) + " not found.<br>" +
        ...
        if (hasAbsMag) {
            ...
            for (int ap = 0; ap < numAps; ap++) {
                ...
                if (srcCol[ap] == ResultsTable.COLUMN_NOT_FOUND) {
                    IJ.showMessage("Table Error", "<html>Table column 'Source-Sky_" + (isRefStar[ap] ? "C" : "T") + (ap + 1) + " does not exist.<br>" +
                            "Can not calculate new target apparent magnitudes.</html>");
                    return;
                }
    ...
    static void storeApertures() {
        ...
        for (int ap = 0; ap < numAps; ap++) {
            if (ap == 0) {
                xString += (isXFITS ? "FITS" : "") + (float) X[ap];
                yString += (isYFITS ? "FITS" : "") + (float) Y[ap];
                absMagString += (float) absMag[ap];
                isRefString += isRefStar[ap];
            } else {
                xString += "," + (float) X[ap];
                yString += "," + (float) Y[ap];
...
//                plotImageCanvas.removeMouseListener(plotMouseListener);
        ...
        if (files.length > 0 && files[0].isFile() && files[0].getName().endsWith(".plotcfg")) {
            ...
            Thread t = new Thread() {
                public void run() {
                    ...
                    try {
                        fileName = dragAndDropFiles[0].getCanonicalPath();
                    } catch (Exception e) {
                        IJ.beep();
                    ...
                    if (panelShiftDown && fileName != null) {
                        ...
                        if (newTable == null) {
                            ...
                            String cfgPath = lastDot > 0 ? fileName.substring(0, lastDot) + ".plotcfg" :
                                    fileName + ".plotcfg";
                            File cfgFile = null;
                            try {
        ...
        try {
            ...
            if (newTable == null) {
                if (!selectAnotherTableCanceled) {
                    IJ.beep();
                    if (fromFile)
                        IJ.showMessage("Unable to open measurement table " + filePath);
                    else
                ...
                if (newColumns.length > 2 && newRows > 0) {
                    ...
                    for (int i = 2; i < oldColumns.length; i++)  //copy all new columns common with old table
                    {
                        ...
                        for (int j = oldRows; j < combinedRows; j++) {
                            combinedTable.setValue(oldColumns[i], j, columnIndex == ResultsTable.COLUMN_NOT_FOUND ?
                                    Double.NaN : newTable.getValueAsDouble(columnIndex, j - oldRows));
                        }
                    }
                    ...
                    for (int i = 2; i < newColumns.length; i++) {
                        for (int j = 0; j < combinedRows; j++) {
                            if (j < newRows)
                                ...
                                combinedTable.setValue(newColumns[i] + appendSourceSuffix, j, Double.NaN);
                        }
                    }
                    errorCode = 11;
                    table.show();
    ...
    static MeasurementTable selectAnotherTable(String firstTableName) {
        ...
        if (tables == null || tables.length == 0) {
            ...
            int j = 0;
            for (int i = 0; i < tables.length; i++)
                if (tables[i].equals(firstTableName) || tables[i].equals("B&C") || tables[i].startsWith("Log") || tables[i].equals("Errors"))
                    j++;
            String[] filteredTables = new String[tables.length - j];
...
//            String dir = OpenDialog.getLastDirectory();
    ...
    static void saveDataSubset(String savePath) {
        ...
        for (int i = 0; i < maxSubsetColumns; i++) {
            if (!subsetColumn[i].trim().equals("")) {
                if (!subsetColumn[i].equals(meridian_flip) && table.getColumnIndex(subsetColumn[i]) == ResultsTable.COLUMN_NOT_FOUND) {
                    IJ.beep();
                    if (IJ.showMessageWithCancel("Save Data Subset Error",
                            "Error: Table column " + subsetColumn[i] + " not found.\nPress OK to correct setting or Cancel to abort save subset.")) {
        ...
        for (int i = 0; i < MeasurementTable.DEFAULT_DECIMALS; i++)
            formatString += "0";
        DecimalFormat output = new DecimalFormat(formatString, IJU.dfs);
...
//            output.setDecimalFormatSymbols(dfs);
        try {
            ...
            for (int row = 0; row < rows; row++) {
                line = (saveRowNumbers ? "" + (row + 1) + delimiter : "") + (saveRowLabels ? table.getLabel(row) + delimiter : "");
                needDelimiter = false;
    ...
    static void createMpcFormatDialog() {
        ...
        if (!(filter.equals(" ") || filter.equals("B") || filter.equals("V") || filter.equals("R") || filter.equals("I") || filter.equals("J") ||
                ...
                filter.equals("w") || filter.equals("y") || filter.equals("z") || filter.equals("N") || filter.equals("T")))
            filter = "B";
        ...
        for (int row = 0; row < table.getCounter(); row++) {
            ...
            if (magCol == MeasurementTable.COLUMN_NOT_FOUND)
                mag = magnitude;
            else
            ...
            if (filter.equals(" ") || Double.isNaN(mag)) {
                ...
                if (useTwoMagPlaces)
                    SMag = twoDigitsTwoPlaces.format(mag);
                else
    ...
    static public void saveDataImageConfig(boolean savePlot, boolean saveConfig, boolean saveData, boolean filenamesProvided,
                                           String imageFormat, String plotPath, String configPath, String dataPath) {
        savePreferences();
        SaveDialog sf = null;
        String outPath = "";
        String outBase = "";
        if (!filenamesProvided) {
            sf = new SaveDialog("Save " + (saveData ? "measurement table data" : "") + (saveData && savePlot ? ", " : "") +
                    (savePlot ? "plot image" : "") + (saveData && savePlot && saveConfig ? "," : "") + ((saveData || savePlot) && saveConfig ? " and " : "") +
                    (saveConfig ? "plot configuration" : ""), MeasurementTable.shorterName(table.shortTitle()), "");
            if (sf.getDirectory() == null || sf.getFileName() == null)
                return;
        ...
        if (savePlot) {
            ...
            if (image == null) {
                IJ.beep();
                IJ.showMessage("No plot image to save");
            } else if (imageFormat.toLowerCase().equals("png")) {
                IJ.runPlugIn(image, "ij.plugin.PNG_Writer", imagepath);
            } else if (imageFormat.toLowerCase().equals("jpg")) {
    ...
    static void saveAll(String format, boolean saveAll) {
        ...
        if (saveAll && (savePlot || saveConfig || saveTable)) {
            saveDataImageConfig(savePlot, saveConfig, saveTable, true, format, outBase + plotSuffix + "." + format, outBase + configSuffix + ".plotcfg",
