    ...
    void parseSexString(String s) {
        ...
        if (fields.length == 1) {  // a string, but just a single number
            ...
            try {
                ...
                second = 60. * (doubleMinute - (double) minute);
            }
        ...
        value = (double) hour + (double) minute / 60. + second / 3600.;
        value = value * sign;
 ...
 UT or local - that's handled by InstantInTime. */
{
    ...
    double Cal2JD() {
        ...
        A = (long) ((y + 0.00000001) / 100.);
        B = 2 - A + (long) ((A + 0.000000001) / 4.);
        ...
        if (is_ut) {
            ...
            if (use_dst == 0) dstInEffect = false;
                ...
                if (use_dst > 0) {  // northern hemisphere logic
                    dstInEffect = jd > DSTBounds[0] & jd < DSTBounds[1];
                }
            ...
            if (use_dst == 0) dstInEffect = false;
                ...
                if (use_dst > 0) {  // northern hemisphere logic
                    dstInEffect = localjd > DSTBounds[2] & localjd < DSTBounds[3];
                }
                if (use_dst < 0) {  // southern hemisphere logic
                    dstInEffect = localjd < DSTBounds[2] | localjd > DSTBounds[3];
                }
...
class Celest implements Cloneable {
    ...
    public boolean equals(Object arg) {
        ...
        if ((arg != null) && (arg instanceof Celest)) {
            Celest c = (Celest) arg;
            return c.Alpha.value == Alpha.value && c.Delta.value == Delta.value &&
                    c.Equinox == Equinox;
        } else return false;
    }
...
//      System.out.printf("Leaving sun w/ wIn %f w %f\n",wIn.when.jd,w.when.jd);
   ...
   /* Set up calculation, then walk in with Newton-Raphson scheme (guess and
    ...
    void Update(WhenWhere wIn) {
        ...
        WhenWhere w = wIn.clone();
        w.when = wIn.when.clone();
        w.where = wIn.where.clone();
        midnight.where = w.where.clone();
        midnight.when = w.when.clone();
        sunrise.where = w.where.clone();
        sunset.where = w.where.clone();
        moonrise.where = w.where.clone();
        moonset.where = w.where.clone();
        eveningTwilight.where = w.where.clone();
        morningTwilight.where = w.where.clone();
        nightcenter.where = w.where.clone();
    ...
    void Update(Observation obs) {
        ...
        if (Math.abs(lastcomputedobs.w.when.jd - obs.w.when.jd) > 5. ||
            ...
            for (i = 0; i < 8; i++) {
                ...
                tabledata[2 * i][1] = ng.sunset.when.localDate.RoundedCalString(3, 0);
...
//      }
        ...
        moonphasefield.setText(String.format(Locale.ENGLISH, "%s", Moon.MoonPhaseDescr(o.w.when.jd)));
...
//   }
            ...
            if (i != 2) {  // skip earth ....
                ...
                if (sepn < 3.) {
                    if (i > 0 & i < 6) {  // Venus through Saturn
                        warningtext = warningtext + String.format(Locale.ENGLISH, "%s - %4.2f deg ", Planets.names[i], sepn);
                        if (sepn < 1.) {
                            warninglevel = 2;
    ...
    class PlanetWindow extends JFrame {
        ...
        PlanetWindow() {
            ...
            for (i = 0; i < 9; i++) {
                if (i != 2) {  // skip earth
                    PlanetDispData[j][0] = Planets.names[i];
...
//          this.setSize(520,260);
        ...
        void DisplayUpdate() {  // assumes computations are already done
            ...
            for (i = 0; i < 9; i++) {
                if (i != 2) {  // skip earth
                    ptable.setValueAt(Planets.names[i], j, 0);
...
//           String dststr = "Standard";
        ...
        double[] pixtoxy(int xpixel, int ypixel) {
            ...
            retvals[0] = xmid + halfwidthx * ((double) (2 * xpixel) / xpix - 1.);
            retvals[1] = ymid + halfwidthy * (1. - (double) (2 * ypixel) / ypix);
        ...
        Celest pixtocelest(int xpixel, int ypixel) {
            ...
            x = xmid + halfwidthx * ((double) (2 * xpixel) / xpix - 1.);
            y = ymid + halfwidthy * (1. - (double) (2 * ypixel) / ypix);
        ...
        void plotplanets() {
            ...
            for (i = 0; i < 9; i++) {
                if (i != 2) {
                    ...
                    puttext(xy[0], xy[1] + 0.01, Planets.names[i], smallfont, smallfontmetrics);
                }
            }
        }
...
//       }
        ...
        void plotAirmass(WhenWhere wIn, Celest cIn, String objectname, Color objcolor) {
            ...
            Observation oairm = new Observation(wIn.clone(),
                    cIn.clone());
