...
public class MeasurementRoi extends Roi {
    ...
    public boolean contains(double x0, double y0) {
        if ((x1 < x2 && (x0 < x1 - radius || x0 > x2 + radius)) || (x1 > x2 && (x0 > x1 + radius || x0 < x2 - radius)) ||
                (y1 < y2 && (y0 < y1 - radius || y0 > y2 + radius)) || (y1 > y2 && (y0 > y1 + radius || y0 < y2 - radius)))
            return false;
    ...
    public void draw(Graphics g) {
        ...
        g.translate(((sx1 + sx2) / 2), ((sy1 + sy2) / 2));
        ...
        if (showMultiLines && !(wAllText + (alen > r ? alen + alen : r + r) < lineLength)) {
            ...
            else if (sDM && showCircle && r > awid && (wDM + w1 >= lineLength - 1))
                yShift -= r - (sFR ? (r < h ? r : h) : 0);
            else if (sDM && (wDM + 2 * alen >= lineLength - 1))
                yShift -= (int) awid - (sFR ? ((int) awid < h ? (int) awid : h) : 0);
            else if (sPA && showCircle && r > awid && (wPA + w1 >= lineLength - 1))
                yShift -= r - (sFR && sDM ? (r < h + h ? r : h + h) : 0) - (!(sFR && sDM) && (sFR || sDM) ? (r < h ? r : h) : 0);
            else if (sPA && (wPA + 2 * alen >= lineLength - 1))
                yShift -= (int) awid - (sFR && sDM ? ((int) awid < h + h ? (int) awid : h + h) : 0) - (!(sFR && sDM) && (sFR || sDM) ? ((int) awid < h ? (int) awid : h) : 0);
            else if (sLen && showCircle && w1 / 2 > awid && (wLength + w1 >= lineLength - 1))
                yShift -= r - (sFR && sDM && sPA ? (r < h + h + h ? r : h + h + h) : 0) -
                        (!(sFR && sDM && sPA) && ((sFR && sDM) || (sDM && sPA) || (sPA && sFR)) ? (r < h + h ? r : h + h) : 0) -
                        (((sFR && !sDM && !sPA) || (!sFR && sDM && !sPA) || (!sFR && !sDM && sPA)) ? (r < h ? r : h) : 0);
            else if (sLen && (wLength + 2 * alen >= lineLength - 1))
                yShift -= (int) awid - (sFR && sDM && sPA ? ((int) awid < h + h + h ? (int) awid : h + h + h) : 0) -
